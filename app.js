// Planejamento de Obra A|W — v6.03.07
...
// Fase 4 — Aparência "congelada": o Gantt inteiro fica acinzentado e claro quando congelado,
// preparando a futura camada de "realizado". Cobre TODAS as barras (ARQ, TEC, obra, disciplinas,
// pré-obra) de uma vez, inclusive as que não têm data-drag. A trava funcional continua sendo
// controlada separadamente em gDragStart/gPopApply.
function _congAplicarVisual(){
  var root=document.getElementById('gantt-root');
  if(!root)return;
  var frozen=_isFrozen();
  // Aplica no container da timeline (as barras vivem dentro de #g-tl-inner)
  var inner=root.querySelector('#g-tl-inner')||root.querySelector('#g-tl-col');
  if(inner){
    if(frozen){
      inner.style.filter='grayscale(1) brightness(1.28) opacity(.92)';
      inner.style.transition='filter .2s';
    }else{
      inner.style.filter='';
    }
  }
  // Cursor "bloqueado" nas barras travadas (feedback ao passar o mouse)
