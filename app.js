...
function getDiasAprovacao(subId){var fase=gSt.projFases[0];if(!fase)return[];var sub=fase.rows.arq.subs[subId];if(!sub)return[];var cfg=getAprovaConfig(subId);if(!cfg.ativo||cfg.dias<=0)return[];var result=[];var cur=new Date(sub.end);while(result.length<cfg.dias){if(!CALENDARIO.isNaoUtil(cur)){result.unshift(G.fmtISO(cur));}if(result.length<cfg.dias){cur=G.addD(cur,-1);if(G.ms(cur)<G.ms(sub.start))break;}else break;}return result;}
