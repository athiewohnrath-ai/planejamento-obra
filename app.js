// Planejamento de Obra A|W — v6.03.52
...
function renderObraFases(){
  ...
  let html = '<table style="width:100%;border-collapse:collapse;font-size:12px;">'
    ...
    + '<th style="padding:7px 10px;text-align:left;font-family:var(--font);font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--txt-muted);border-bottom:1px solid var(--border);width:44px;">Fase</th>'    + '<th style="padding:7px 10px;text-align:left;font-family:var(--font);font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--txt-muted);border-bottom:1px solid var(--border);width:210px;">Pré-Obra</th>'    + '<th style="padding:7px 10px;text-align:left;font-family:var(--font);font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--txt-muted);border-bottom:1px solid var(--border);">Template de Obra</th>'    + '<th style="padding:7px 10px;text-align:center;font-family:var(--font);font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--txt-muted);border-bottom:1px solid var(--border);width:32px;"></th>'    + '</tr></thead><tbody>';
  ...
  for (let i = 0; i < n; i++) {
    ...
    const ressincBtn = faseMod
      ? '<button onclick="_tplRessincronizar('+i+')" title="Ressincronizar" style="width:26px;height:26px;border:1px solid #C07820;background:rgba(192,120,32,.08);color:#C07820;border-radius:4px;cursor:pointer;font-size:13px;">↺</button>'      : '<div style="width:26px;"></div>';
...
function _pImp(aba){_pAba=aba;document.getElementById('modal-print')?.remove();let ps=document.getElementById('_pPageStyle');if(!ps){ps=document.createElement('style');ps.id='_pPageStyle';document.head.appendChild(ps);}ps.textContent=aba!=='arquitetura'?'@media print{@page{size:A3 landscape;margin:8mm;}#_pFrame{display:block!important;}body>*:not(#_pFrame){display:none!important;}}':'@media print{@page{size:A4 portrait;margin:8mm;}#_pFrame{display:block!important;}body>*:not(#_pFrame){display:none!important;}}';let pf=document.getElementById('_pFrame');if(!pf){pf=document.createElement('div');pf.id='_pFrame';pf.style.display='none';document.body.appendChild(pf);}pf.innerHTML='';const cod=ESTADO.meta?.codigo||'';const nome=ESTADO.meta?.nome||'';const hdr='<div style="display:flex;align-items:center;justify-content:space-between;padding:6px 12px;background:#1A2535;flex-shrink:0;"><span style="font-family:Oswald,sans-serif;font-size:10px;font-weight:700;color:#00DEDB;">'+(cod&&nome?cod+' \u2014 '+nome:cod||nome||'Planejamento de Obra')+'</span><span style="font-family:Oswald,sans-serif;font-size:8px;color:rgba(255,255,255,.35);">'+new Date().toLocaleDateString('pt-BR')+'</span></div>';if(aba==='cronograma'){const src=document.getElementById('gantt-root');if(src&&src.children.length){pf.style.cssText='display:none;width:100%;height:100vh;flex-direction:column;background:#fff;';pf.innerHTML=hdr+'<div style="flex:1;overflow:hidden;">'+src.innerHTML+'</div>';}}else if(aba==='efetivo'){const src=document.getElementById('pane-efetivo');if(src&&src.querySelector('svg')){pf.style.cssText='display:none;width:100%;height:100vh;flex-direction:column;background:#fff;';pf.innerHTML=hdr+'<div style="flex:1;overflow:hidden;">'+src.innerHTML+'</div>';}}else{pf.style.cssText='display:none;width:100%;background:#fff;font-family:Oswald,sans-serif;';let rows='';(gSt.projFases||[]).forEach(fase=>{const nf=fase.nome?.trim()||'Fase '+fase.id;rows+='<tr><td colspan="5" style="padding:5px 8px;font-size:10px;font-weight:700;color:#fff;background:#1A2535;letter-spacing:.06em;">'+nf.toUpperCase()+'</td></tr>';G.SUB_IDS.forEach(id=>{const s=fase.rows.arq?.subs?.[id];if(!s)return;rows+='<tr style="border-bottom:1px solid #F0F2F4;"><td style="padding:3px 8px 3px 14px;font-size:9px;font-weight:700;color:#1A5294;">ARQ</td><td style="padding:3px 8px;font-size:10px;color:#1A2535;">'+(G.SUB_NAMES[id]||id)+'</td><td style="padding:3px 8px;font-size:9px;color:#666;text-align:center;">'+G.fmtBR(s.start instanceof Date ? s.start : G.parseD(s.start))+'</td><td style="padding:3px 8px;font-size:9px;color:#666;text-align:center;">'+G.fmtBR(s.end instanceof Date ? s.end : G.parseD(s.end))+'</td><td style="padding:3px 8px;font-size:9px;color:#999;text-align:center;">'+CALENDARIO.contarDU(new Date(s.start),new Date(s.end))+' DU</td></tr>';});G.TEC_IDS.forEach(id=>{const s=fase.rows.tec?.subs?.[id];if(!s)return;rows+='<tr style="border-bottom:1px solid #F0F2F4;"><td style="padding:3px 8px 3px 14px;font-size:9px;font-weight:700;color:#2A7A5A;">TEC</td><td style="padding:3px 8px;font-size:10px;color:#1A2535;">'+(G.TEC_NAMES[id]||id)+'</td><td style="padding:3px 8px;font-size:9px;color:#666;text-align:center;">'+G.fmtBR(s.start instanceof Date ? s.start : G.parseD(s.start))+'</td><td style="padding:3px 8px;font-size:9px;color:#666;text-align:center;">'+G.fmtBR(s.end instanceof Date ? s.end : G.parseD(s.end))+'</td><td style="padding:3px 8px;font-size:9px;color:#999;text-align:center;">'+CALENDARIO.contarDU(new Date(s.start),new Date(s.end))+' DU</td></tr>';});});pf.innerHTML=hdr+'<table style="width:100%;border-collapse:collapse;"><thead><tr style="background:#F4F6F8;"><th style="padding:5px 8px;font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#8A95A8;text-align:left;">Grupo</th><th style="padding:5px 8px;font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#8A95A8;text-align:left;">Etapa</th><th style="padding:5px 8px;font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#8A95A8;text-align:center;">In\u00edcio</th><th style="padding:5px 8px;font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#8A95A8;text-align:center;">T\u00e9rmino</th><th style="padding:5px 8px;font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#8A95A8;text-align:center;">Dur.</th></tr></thead><tbody>'+rows+'</tbody></table>';}setTimeout(()=>{window.print();setTimeout(()=>{pf.innerHTML='';pf.style.display='none';},2000);},150);}document.addEventListener("DOMContentLoaded",()=>{buildVizSidebar()}),document.addEventListener("keydown",t=>{"Escape"===t.key&&fecharModalCfg({target:document.getElementById("modal-cfg-overlay")})});const EARQ_H=8,EARQ_FASE_COLORS=["#185FA5","#2A5AA8","#8A3AA8","#2A8A5A"];function earqGetEstado(){return ESTADO.equipeARQ||(ESTADO.equipeARQ={chArq:120,chDir:280,chGer:180,cotas:{}}),ESTADO.equipeARQ}function earqInit(){const t=earqGetEstado(),e=document.getElementById("earq-ch-arq"),o=document.getElementById("earq-ch-dir"),a=document.getElementById("earq-ch-ger");e&&(e.value=t.chArq??120),o&&(o.value=t.chDir??280),a&&(a.value=t.chGer??180),earqBuildTable(),earqRecalc()}function earqBuildTable(){  const t=document.getElementById("earq-thead"),e=document.getElementById("earq-tbody"),o=document.getElementById("earq-tfoot");  // Aplicar max-width no container da tabela  const _tblWrap=document.getElementById("earq-table-wrap");  if(_tblWrap) _tblWrap.style.cssText="max-width:1100px;overflow-x:auto;";  if(!t||!e||!o)return;  const cod=ESTADO.meta?.codigo||'', nome=ESTADO.meta?.nome||'';  const proj=cod&&nome?cod+' — '+nome:cod||nome||'';  // Cabeçalho do relatório (aparece ao imprimir)  const rptHdr=document.getElementById('earq-rpt-header');  if(rptHdr){    const chArq=parseFloat(document.getElementById('earq-ch-arq')?.value)||0;    const chDir=parseFloat(document.getElementById('earq-ch-dir')?.value)||0;    const chGer=parseFloat(document.getElementById('earq-ch-ger')?.value)||0;    rptHdr.innerHTML=`      <div style="display:flex;align-items:flex-end;justify-content:space-between;border-bottom:3px solid #1A5294;padding-bottom:12px;margin-bottom:4px;">        <div>          <div style="font-family:var(--font);font-size:18px;font-weight:700;color:#1A2535;letter-spacing:.04em;text-transform:uppercase;">Equipe de Arquitetura</div>          <div style="font-family:var(--body);font-size:11px;color:#6A7A8A;margin-top:3px;">${proj?'<strong>'+proj+'</strong> · ':''} Estimativa de alocação e custos</div>        </div>        <div style="text-align:right;font-family:var(--body);font-size:9px;color:#8A95A8;line-height:1.6;">          <div>ch Arq: <strong>R$ ${chArq}/h</strong></div>          <div>ch Dir: <strong>R$ ${chDir}/h</strong></div>          <div>ch Ger: <strong>R$ ${chGer}/h</strong></div>          <div>Emitido: <strong>${new Date().toLocaleDateString('pt-BR')}</strong></div>        </div>      </div>`;  }  const TH=(t,e="")=>`<th style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.04em;padding:6px 10px;text-align:right;border-bottom:2px solid #C8D4E8;white-space:nowrap;${e}">${t}</th>`;  const THl=(t,e="")=>TH(t,"text-align:left;"+e);  t.innerHTML=`<tr style="background:#F0F4FA;">${THl("Atividade","min-width:130px;")} ${THl("Fase","")} ${TH("Início","")} ${TH("Fim","")} ${TH("DU","")} ${TH("arq/dia","background:#E8F0FC;color:#0C447C;")} ${TH("horas ARQ","background:#E8F0FC;color:#0C447C;")} ${TH("cota Dir. (h)","background:#F0EBF8;color:#3C3489;")} ${TH("custo Dir.","background:#F0EBF8;color:#3C3489;")} ${TH("cota Ger. (h)","background:#E4F2EB;color:#085041;")} ${TH("custo Ger.","background:#E4F2EB;color:#085041;")} ${TH("Custo total","")} </tr>`;  const a=earqGetEstado();let n=earqGetRows();  e.innerHTML="";let _prevFaseIdx=-1;  n.forEach((t,idx)=>{    const n=a.cotas[t.key]||{dir:0,ger:0};    const r=EARQ_FASE_COLORS[(t.faseIdx??0)%EARQ_FASE_COLORS.length];    // Separador de fase    if(t.faseIdx!==_prevFaseIdx){      _prevFaseIdx=t.faseIdx;      const faseRow=document.createElement("tr");      faseRow.innerHTML=`<td colspan="12" style="padding:6px 10px 2px;font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:${r};background:${r}12;border-bottom:1px solid ${r}44;">${t.faseLabel}</td>`;      e.appendChild(faseRow);    }    // Calcular média e datas    const _arqDias=t.arqDiasReal!=null?t.arqDiasReal:t.arq*t.du;    const _media=t.du>0?(_arqDias/t.du).toFixed(1):'0';    const _isAdj=t.arqDiasReal!=null&&Math.abs(t.arqDiasReal-t.arq*t.du)>0.001;    const _fase=gSt.projFases.find(f=>f.nome===t.faseLabel||('F'+(f.id))===t.faseLabel);    const _phId=parseInt(t.key.split('/')[0]);    const _phase=gSt.projFases.find(f=>f.id===_phId);    const _sub=_phase?.rows?.arq?.subs?.[t.key.split('/')[2]];    const _ini=_sub?G.fmtBR(_sub.start):'—';    const _fim=_sub?G.fmtBR(_sub.end):'—';    const s=document.createElement("tr");    s.style.cssText=`border-bottom:1px solid #E8EDF4;${idx%2===0?'background:#FAFBFD;':'background:#fff;'}`;    s.innerHTML=`      <td style="padding:7px 10px;font-weight:600;color:#1A2535;">${t.nome}</td>      <td style="padding:7px 10px;"></td>      <td style="padding:7px 10px;text-align:right;color:#6A7A8A;font-size:10px;">${_ini}</td>      <td style="padding:7px 10px;text-align:right;color:#6A7A8A;font-size:10px;">${_fim}</td>      <td style="padding:7px 10px;text-align:right;color:#6A7A8A;">${t.du}</td>      <td style="padding:7px 10px;text-align:right;background:#EEF2FA;">        <div style="display:flex;align-items:center;gap:3px;justify-content:flex-end;">          <button onclick="earqAdjArq('${t.key}',-1)" class="earq-no-print" style="width:18px;height:18px;border:1px solid #B5D4F4;border-radius:3px;background:#fff;cursor:pointer;font-size:12px;color:#185FA5;display:flex;align-items:center;justify-content:center;line-height:1;padding:0;">−</button>          <span id="earq-v-arq-${t.key}" style="font-size:12px;font-weight:800;color:${_isAdj?'#E07020':'#185FA5'};min-width:28px;text-align:center;" title="${_isAdj?'Média real: '+_media+' arq/dia':'Padrão: '+t.arq+' arq/dia'}">${_isAdj?_media:t.arq}${_isAdj?'<span style="font-size:8px;color:#E07020;margin-left:1px;">~</span>':''}</span>          <button onclick="earqAdjArq('${t.key}',1)" class="earq-no-print" style="width:18px;height:18px;border:1px solid #B5D4F4;border-radius:3px;background:#fff;cursor:pointer;font-size:12px;color:#185FA5;display:flex;align-items:center;justify-content:center;line-height:1;padding:0;">+</button>        </div>      </td>      <td id="earq-h-arq-${t.key}" style="padding:7px 10px;text-align:right;font-weight:700;color:${_isAdj?'#E07020':'#185FA5'};background:#EEF2FA;">—</td>      <td style="padding:7px 10px;background:#F5F0FC;"><input type="number" id="earq-dir-${t.key}" value="${t.cotaDir??n.dir}" min="0" step="1" oninput="earqSaveCota('${t.key}');earqRecalc()" style="width:50px;font-family:var(--font);font-size:11px;font-weight:700;border:1px solid #AFA9EC;border-radius:4px;padding:3px 5px;text-align:right;background:#fff;color:#3C3489;"></td>      <td id="earq-c-dir-${t.key}" style="padding:7px 10px;text-align:right;font-weight:700;color:#534AB7;background:#F5F0FC;">—</td>      <td style="padding:7px 10px;background:#E8F6F0;"><input type="number" id="earq-ger-${t.key}" value="${t.cotaGer??n.ger}" min="0" step="1" oninput="earqSaveCota('${t.key}');earqRecalc()" style="width:50px;font-family:var(--font);font-size:11px;font-weight:700;border:1px solid #5DCAA5;border-radius:4px;padding:3px 5px;text-align:right;background:#fff;color:#085041;"></td>      <td id="earq-c-ger-${t.key}" style="padding:7px 10px;text-align:right;font-weight:700;color:#0F6E56;background:#E8F6F0;">—</td>      <td id="earq-c-tot-${t.key}" style="padding:7px 10px;text-align:right;font-weight:700;color:#0C447C;">—</td>`;    e.appendChild(s);  });  o.innerHTML=`<tr style="border-top:2px solid #1A5294;background:#EEF2FA;">    <td colspan="5" style="padding:9px 10px;font-weight:700;color:#1A2535;">TOTAL GERAL</td>    <td id="earq-tot-ad" style="padding:9px 10px;text-align:right;font-weight:800;color:#0C447C;font-size:13px;">—</td>    <td id="earq-tot-arq" style="padding:9px 10px;text-align:right;font-weight:800;color:#0C447C;font-size:13px;">—</td>    <td id="earq-tot-dir-h" style="padding:9px 10px;text-align:right;font-weight:700;color:#534AB7;">—</td>    <td id="earq-tot-dir-c" style="padding:9px 10px;text-align:right;font-weight:700;color:#534AB7;">—</td>    <td id="earq-tot-ger-h" style="padding:9px 10px;text-align:right;font-weight:700;color:#0F6E56;">—</td>    <td id="earq-tot-ger-c" style="padding:9px 10px;text-align:right;font-weight:700;color:#0F6E56;">—</td>    <td id="earq-tot-geral" style="padding:9px 10px;text-align:right;font-weight:800;color:#1A5294;font-size:13px;">—</td>  </tr>`;
...
gObraRow(t,o,e,r,i),t.expanded&&(t.disciplinas||[]).forEach((a,n)=>{a&&a.ativo&&gDiscRow(t,a,n,o,e,r,i)}),spacer(4)}}),spacer(),!s){if(ESTADO.cfg.preObra&&gSt.obraFases.length){const t=parseInt(ESTADO.cfg.preObraDias)||10;let a=new Date(gSt.obraFases[0].obra.start);for(;CALENDARIO.isNaoUtil(a);)a=G.addD(a,-1);let n=new Date(a),s=0;for(;s<t;)n=G.addD(n,-1),CALENDARIO.isNaoUtil(n)||s++;for(;CALENDARIO.isNaoUtil(n);)n=G.addD(n,1);const d=gPx(n),l=Math.max(G.diff(n,a)*gSt.dayW,2*gSt.dayW),c=darkenHex(COR.OBRA_MOM,.6),p="rgba(122,74,16,.06)";r.push(`<div style="height:${G.ROW_H}px;background:${p};display:flex;align-items:center;padding:0 5px 0 8px;border-bottom:1px solid rgba(122,74,16,.15);border-right:2px solid #C0C8D4;font-size:9px;font-weight:700;color:${c};text-transform:uppercase;letter-spacing:.06em;gap:3px;"><span style="width:5px;height:5px;border-radius:1px;background:${c};flex-shrink:0;opacity:.5;"></span><span>Pré Obra</span><span style="margin-left:auto;opacity:.6;">${t}DU</span></div>`),i.push(`<div style="height:${G.ROW_H}px;background:${p};position:relative;border-bottom:1px solid rgba(122,74,16,.15);overflow:hidden;">${gGridLines(o,e,!1)}<div style="position:absolute;left:${d}px;top:4px;height:${G.ROW_H-8}px;width:${Math.max(l,8)}px;background:repeating-linear-gradient(135deg,${c}44 0,${c}44 4px,transparent 4px,transparent 8px);border:1px solid ${c}66;border-radius:4px;"></div></div>`)}phaseHdr("Obra",darkenHex(COR.OBRA_MOM,.65));const t=gSt.obraFases.length>1;gSt.obraFases.forEach((a,_fi)=>{if(t){const t=gObraFaseLabel(a),n=darkenHex(COR.OBRA_MOM,.72),s=a.expanded?"▲":"▼";r.push(`<div style="height:20px;background:${n};display:flex;align-items:center;padding:0 6px 0 12px;font-size:9px;font-weight:700;color:#fff;text-transform:uppercase;letter-spacing:.06em;border-right:2px solid ${darkenHex(COR.OBRA_MOM,.6)};gap:4px;"><span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${t}</span><span onclick="gEditFaseNome('obra',${a.id},'${(a.nome||"").replace(/'/g,"")}')" style="cursor:pointer;opacity:.7;font-size:10px;" title="Renomear">✎</span><button onclick="gToggleObraExpand(${a.id})" style="background:rgba(255,255,255,.15);border:none;color:#fff;border-radius:3px;cursor:pointer;font-size:9px;padding:1px 5px;flex-shrink:0;">${s}</button></div>`),i.push(`<div style="height:20px;background:linear-gradient(90deg,${n}CC,${n}22);position:relative;">${gGridLines(o,e,!1)}</div>`)}// Pré-obra da fase(function(obraFase, obraFaseIdx){  var pc=ESTADO.cfg.obraFases&&ESTADO.cfg.obraFases[obraFaseIdx]&&ESTADO.cfg.obraFases[obraFaseIdx].preObra;
  ...
  if(tpl){
    ...
    ds.forEach(function(pd,pi){
      ...
      r.push('<div style="height:'+G.SUB_H+'px;background:#FEFCF5;display:flex;align-items:center;padding:0 8px;border-bottom:1px solid rgba(122,74,16,.12);border-right:2px solid #C0C8D4;font-size:9px;color:#6A3810;gap:4px;overflow:hidden;"><span style="width:4px;height:4px;border-radius:1px;background:'+dc+';flex-shrink:0;"></span><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+(pd.label||pd.id)+'</span></div>');      i.push('<div style="height:'+G.SUB_H+'px;background:#FEFCF5;position:relative;border-bottom:1px solid rgba(122,74,16,.12);overflow:hidden;">'+gGridLines(o,e,false)+(xwC>0?'<div style="position:absolute;top:5px;left:'+xiC+'px;height:'+(G.SUB_H-10)+'px;width:'+xwC+'px;background:'+dc+';border-radius:2px;opacity:.85;"></div>':''  )+'</div>');    });
  }
  spacer(2);})(a, _fi);
...
const r=document.createElementNS("http://www.w3.org/2000/svg","rect");r.setAttribute("x",p+o*l),r.setAttribute("y",u),r.setAttribute("width",l),r.setAttribute("height",m),r.setAttribute("fill","transparent"),r.style.cursor="default";const i=["Dom","Seg","Ter","Qua","Qui","Sex","Sáb"][e.dow],s=t.dateToMod?t.dateToMod.get(a)||"?":e.mod;r.addEventListener("mouseenter",o=>{let r=`<div style="font-family:var(--font);font-size:10px;font-weight:700;color:#00C8FF;margin-bottom:6px;">${i} ${fmtDateEft(e.date)} — M${s}</div>`;t.disc.forEach((e,o)=>{const n=t.profByDay[o][a]?.total||0;n&&(r+=`<div style="display:flex;align-items:center;gap:5px;margin:2px 0;"><div style="width:8px;height:8px;border-radius:2px;background:${HEAT_CFG.discColors[o%HEAT_CFG.discColors.length]};flex-shrink:0;"></div><span style="flex:1;font-size:10px;">${e.label}</span><span style="font-size:10px;font-weight:700;color:#D4A848;">${n}</span></div>`)}),r+=`<div style="border-top:1px solid rgba(255,255,255,.15);margin-top:5px;padding-top:4px;font-size:11px;color:#D4A848;">Total: <strong>${n}</strong> prof/dia</div>`,showEftTip(o,r)}),r.addEventListener("mousemove",moveEftTip),r.addEventListener("mouseleave",hideEftTip),A.appendChild(r)}),n.appendChild(A)})});let b=!1;const redrawAll=()=>{m.forEach(t=>t())};new ResizeObserver(()=>{b||(b=!0,requestAnimationFrame(()=>{b=!1,redrawAll()}))}).observe(c),requestAnimationFrame(redrawAll)}function renderFornecedores(){const t=document.getElementById("pane-fornecedores");t.innerHTML="",t.style.display="flex",t.style.flexDirection="column",t.style.overflow="hidden";const e=gSt.obraFases[0];if(!e)return void(t.innerHTML='<div class="pane-placeholder"><div class="ph-title">Configure a obra primeiro</div></div>');const o=e.disciplinas||[],a=G.SEG_N;window._fornData||(window._fornData={});const n=document.createElement("div");n.style.cssText="flex-shrink:0;display:flex;align-items:center;justify-content:space-between;padding:10px 16px;border-bottom:1px solid var(--border);background:var(--bg-panel);",n.innerHTML='<div><span style="font-family:var(--font);font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--txt);">Fornecedores por Disciplina</span><span style="font-family:var(--font);font-size:9px;color:var(--txt-dim);margin-left:10px;">Clique em uma célula para editar</span></div>',t.appendChild(n);const r=document.createElement("div");r.style.cssText="flex:1;overflow:auto;";const i=document.createElement("table");i.style.cssText="width:100%;border-collapse:collapse;font-family:var(--body);font-size:11px;";const s=document.createElement("thead");let d='<tr style="background:var(--bg-surface2);position:sticky;top:0;z-index:5;"><th style="width:140px;padding:8px 12px;text-align:left;font-family:var(--font);font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--txt-muted);border-bottom:2px solid var(--border-md);border-right:2px solid var(--border-md);">Disciplina</th><th style="width:160px;padding:8px 10px;text-align:left;font-family:var(--font);font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--txt-muted);border-bottom:2px solid var(--border-md);border-right:1px solid var(--border);">Fornecedor</th><th style="width:100px;padding:8px 10px;text-align:left;font-family:var(--font);font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--txt-muted);border-bottom:2px solid var(--border-md);border-right:1px solid var(--border);">Responsável</th>';for(let t=1;t<=a;t++){const e=3===t||5===t;d+=`<th style="width:50px;padding:8px 4px;text-align:center;font-family:var(--font);font-size:9px;font-weight:700;color:${e?"#B83418":"var(--txt-muted)"};border-bottom:2px solid var(--border-md);border-right:${e?"2px solid #B83418":"1px solid var(--border)"};">M${t}</th>`}d+='<th style="padding:8px 10px;text-align:left;font-family:var(--font);font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--txt-muted);border-bottom:2px solid var(--border-md);">Observações</th></tr>',s.innerHTML=d,i.appendChild(s);const l=document.createElement("tbody");// Adicionar linha de Pré-Obra quando ativa
  ...
  if(_poDiscs.length>0){
    ...
    _poHdr.innerHTML='<td colspan="'+(4+a)+'" style="padding:5px 12px;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#8A6010;background:#FEF6E4;border-bottom:1px solid #E8D088;">Pré-Obra</td>';    l.appendChild(_poHdr);    _poDiscs.forEach(function(pd,po){
      const _poKey='po_'+po;
...
function ouImprimir() {
  ...
  discs.forEach((disc, di) => {
    ...
    rows += '<tr style="background:#F4F6F8;">'
      + '<td colspan="11" style="padding:5px 8px;font-family:Oswald,sans-serif;font-size:11px;font-weight:700;color:'+discColor+';border:0.5px solid #D8DCE4;border-left:3px solid '+discColor+';">'+disc.label+'</td>'      + '</tr>';    (disc.tasks || []).forEach(task => {
      ...
      const modCells = Array.from({length:8},(_,i)=>i+1).map(m => {
        ...
        const content = prof > 0
          ...
          : '<span style="font-size:8px;color:#6B3000;">'+pct+'%</span>';
        return '<td style="padding:3px 4px;text-align:center;font-size:9px;border:0.5px solid #D8DCE4;'+bg+'white-space:nowrap;">'+content+'</td>';      }).join('');
      rows += '<tr>'
...
<div style="display:flex;align-items:flex-end;justify-content:space-between;padding-bottom:8px;border-bottom:2.5px solid #00DEDB;margin-bottom:12px;">
  <div>
    ...
    <div style="font-family:Oswald,sans-serif;font-size:22px;font-weight:700;text-transform:uppercase;color:#1A2535;line-height:1;">Visão Unificada <span style="color:#00DEDB;">${nomeFase}</span></div>    <div style="font-size:10px;color:#5A6275;margin-top:3px;">${cod?cod+' · ':''}${nomeProj}</div>  </div>  <div style="text-align:right;font-size:9px;color:#9AA0AF;line-height:2;">    ${gi?`<div><strong style="color:#1A2535;">GI:</strong> ${gi}</div>`:''}    ${gp?`<div><strong style="color:#1A2535;">GP:</strong> ${gp}</div>`:''}    <div><strong style="color:#1A2535;">Emitido em:</strong> ${hoje}</div>  </div></div>

<!-- TABELA -->
<table>
  <thead>
    <tr style="background:#1A2535;">
      <th style="padding:6px 8px;text-align:left;font-family:Oswald,sans-serif;font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#9AA0AF;border:0.5px solid #2A3545;min-width:130px;">Disciplina / Tarefa</th>      <th style="padding:6px 4px;text-align:center;font-family:Oswald,sans-serif;font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#9AA0AF;border:0.5px solid #2A3545;width:44px;">Tipo</th>      <th style="padding:6px 4px;text-align:center;font-family:Oswald,sans-serif;font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#9AA0AF;border:0.5px solid #2A3545;width:38px;">Efet.</th>      ${Array.from({length:8},(_,i)=>'<th style="padding:6px 4px;text-align:center;font-family:Oswald,sans-serif;font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#9AA0AF;border:0.5px solid #2A3545;width:34px;">M'+(i+1)+'</th>').join('')}      <th style="padding:6px 4px;text-align:center;font-family:Oswald,sans-serif;font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#9AA0AF;border:0.5px solid #2A3545;width:38px;">Total</th>    </tr>  </thead>  <tbody>${rows}</tbody></table>
...
<div class="no-print" style="position:fixed;bottom:20px;right:20px;display:flex;gap:10px;">
  <button onclick="window.close()" style="height:36px;padding:0 16px;background:#F4F6F8;border:1px solid #D8DCE4;border-radius:6px;font-family:Oswald,sans-serif;font-size:10px;font-weight:700;color:#5A6275;cursor:pointer;letter-spacing:.06em;text-transform:uppercase;">Fechar</button>  <button onclick="window.print()" style="height:36px;padding:0 20px;background:#00DEDB;border:none;border-radius:6px;font-family:Oswald,sans-serif;font-size:10px;font-weight:700;color:#0D1117;cursor:pointer;letter-spacing:.06em;text-transform:uppercase;">🖨 Imprimir / PDF</button></div>
...
function ouRender() {
  ...
  discs.forEach((disc, di) => {
    ...
    (disc.tasks || []).forEach((task, ti) => {
      ...
      html += `<input type="text" value="${task.n.replace(/"/g,'\"')}" onchange="ouSetNome(${di},${ti},this.value)" style="width:100%;border:none;background:transparent;font-size:11px;font-family:var(--body);">`;      html += `</td>`;
      ...
      // Efetivo
      html += `<td style="padding:3px;border:1px solid var(--border);text-align:center;">`;
      html += `<input type="number" min="0" max="99" value="${task.prof||0}" onchange="ouSetProf(${di},${ti},this.value)" style="width:44px;border:1px solid var(--border);border-radius:3px;text-align:center;font-size:11px;padding:2px 0;">`;      html += `</td>`;
      ...
      // Remover tarefa
      html += `<td style="padding:3px;border:1px solid var(--border);text-align:center;">`;
      html += `<button onclick="ouRemoverTarefa(${di},${ti})" style="background:none;border:1px solid #E08080;color:#C04040;border-radius:3px;padding:2px 5px;font-size:11px;cursor:pointer;">✕</button>`;      html += `</td></tr>`;    });  });
  ...
  html += `<button onclick="ouAdicionarDisc()" style="background:var(--accent);color:#fff;border:none;padding:6px 16px;border-radius:5px;font-size:11px;cursor:pointer;font-weight:600;">+ Nova Disciplina</button>`;  html += `</td></tr>`;
  ...
  var discsHtml = discs.map(function(disc, di) {
    ...
    var tasksHtml = tableHdr + (disc.tasks||[]).map(function(task, ti) {
      ...
      return '<div style="display:grid;grid-template-columns:1fr 90px 70px 28px;gap:4px;padding:5px 10px 5px 36px;border-bottom:1px solid rgba(0,0,0,.04);align-items:center;">'
        ...
        + 'style="height:26px;padding:0 4px;border:0.5px solid var(--border);border-radius:4px;background:var(--bg-surface2);color:var(--txt);font-size:10px;font-family:var(--font);font-weight:700;width:100%;">'        + '<option value="SERVIÇO" '+(isPrep?'':'selected')+'>SERVIÇO</option>'        + '<option value="PREP" '+(isPrep?'selected':'')+'>PREP</option>'        + '</select>'        + '<input type="number" min="1" max="50" value="'+(task.prof||2)+'" '        + 'onchange="ESTADO.preObraCustom['+faseIdx+'].disciplinas['+di+'].tasks['+ti+'].prof=parseInt(this.value)||1;onCfgChange();gRender();" '        + 'style="height:26px;padding:0 4px;border:0.5px solid var(--border);border-radius:4px;background:var(--bg-surface2);color:var(--txt);font-size:11px;text-align:center;width:100%;">'        + '<button onclick="_poRemoverTarefa('+faseIdx+','+di+','+ti+')" '        + 'style="width:24px;height:24px;background:none;border:none;cursor:pointer;font-size:13px;color:var(--txt-dim);">✕</button>'        + '</div>';    }).join('');
    ...
    return '<div data-po-disc="'+di+'" draggable="true" style="border-radius:8px;margin-bottom:10px;overflow:hidden;border:1px solid rgba(0,0,0,.08);">'
      ...
      + 'style="height:24px;padding:0 8px;background:rgba(180,20,20,.22);border:1px solid rgba(255,255,255,.3);border-radius:4px;font-size:9px;font-weight:700;color:#fff;cursor:pointer;flex-shrink:0;font-family:var(--font);">✕ Disc.</button>'      + '</div>'      + '<div style="background:var(--bg-surface);">' + tasksHtml      + '<div style="padding:6px 10px 6px 36px;"><button onclick="_poAdicionarTarefa('+faseIdx+','+di+')" '      + 'style="font-size:10px;color:'+dc+';background:none;border:none;cursor:pointer;font-family:var(--font);font-weight:700;">+ Tarefa</button></div>'      + '</div></div>';  }).join('');

  ov.innerHTML = '<div style="background:var(--bg-panel);border-radius:12px;box-shadow:0 24px 64px rgba(0,0,0,.45);width:100%;max-width:820px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;">'    + '<div style="display:flex;align-items:center;gap:10px;padding:14px 20px;border-bottom:1px solid var(--border);flex-shrink:0;background:var(--bg-surface2);">'    + '<div style="width:6px;height:22px;border-radius:2px;background:#C07820;flex-shrink:0;"></div>'    + '<div style="flex:1;">'    + '<div style="font-family:var(--font);font-size:13px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:var(--txt);">Pré-Obra · Fase '+(faseIdx+1)+'</div>'    + '<div style="font-size:10px;color:var(--txt-muted);margin-top:1px;">Edição específica deste cronograma — não altera o template</div>'    + '</div>'    + '<label style="display:flex;align-items:center;gap:6px;font-size:10px;color:var(--txt-muted);flex-shrink:0;">Duração (DU)'    + '<input type="number" min="1" max="30" value="'+custom.du+'" '    + 'onchange="ESTADO.preObraCustom['+faseIdx+'].du=parseInt(this.value)||5;ESTADO.cfg.obraFases['+faseIdx+'].preObra.du=parseInt(this.value)||5;onCfgChange();gRender();" '    + 'style="width:52px;height:28px;padding:0 6px;border:1px solid var(--border);border-radius:5px;background:var(--bg-surface);color:var(--txt);font-family:var(--font);font-size:12px;font-weight:700;text-align:center;"></label>'    + '<button onclick="fecharModalPreObra()" style="width:30px;height:30px;background:var(--bg-surface);border:1px solid var(--border);border-radius:5px;cursor:pointer;font-size:14px;color:var(--txt-muted);">✕</button>'    + '</div>'    + '<div style="flex:1;overflow-y:auto;padding:16px 20px;">' + discsHtml    + '<button onclick="_poAdicionarDisc('+faseIdx+')" '    + 'style="width:100%;height:34px;background:var(--bg-surface2);border:1px dashed var(--border);border-radius:6px;font-family:var(--font);font-size:10px;font-weight:700;color:var(--txt-muted);cursor:pointer;">+ Adicionar Disciplina</button>'    + '</div>'    + '<div style="padding:12px 20px;border-top:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">'    + '<button onclick="_poResetarCustom('+faseIdx+')" style="font-size:10px;color:var(--txt-dim);background:none;border:none;cursor:pointer;font-family:var(--font);">↺ Restaurar template</button>'    + '<div style="display:flex;gap:8px;">'    + '<button onclick="fecharModalPreObra()" style="height:32px;padding:0 16px;background:var(--bg-surface2);border:1px solid var(--border);border-radius:6px;font-family:var(--font);font-size:10px;font-weight:700;color:var(--txt-muted);cursor:pointer;">Cancelar</button>'    + '<button onclick="_poSalvar('+faseIdx+')" style="height:32px;padding:0 20px;background:var(--accent);border:none;border-radius:6px;font-family:var(--font);font-size:10px;font-weight:700;color:#0D1117;cursor:pointer;">✓ Salvar</button>'    + '</div></div></div>';
...
function _poOpenDiscModal(faseIdx, di) {
  ...
  function render() {
    ...
    ov.innerHTML = '<div style="background:#FFF8F0;border-radius:12px;box-shadow:0 8px 40px rgba(0,0,0,.3);width:100%;max-width:520px;max-height:85vh;display:flex;flex-direction:column;overflow:hidden;">'      + '<div style="height:5px;background:linear-gradient(to right,'+_discCors[0]+','+_discCors[1]+');border-radius:12px 12px 0 0;flex-shrink:0;"></div>'      + '<div style="display:flex;align-items:center;justify-content:space-between;padding:12px 18px 10px;border-bottom:1px solid #EEE4C8;">'      + '<div>'      + '<div style="font-family:var(--font,sans-serif);font-size:15px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#3A2800;">'+disc.label+'</div>'      + '<div style="font-size:10px;color:#9A8A6A;margin-top:2px;">'+(tasks.length)+' atividade(s) configurada(s)</div>'      + '</div>'      + '<button onclick="document.getElementById(\'modal-po-disc\').remove();_poRenderModal('+faseIdx+');" '      + 'style="width:28px;height:28px;background:none;border:none;cursor:pointer;font-size:18px;color:#9A8A6A;">✕</button>'      + '</div>'      + '<div style="flex:1;overflow-y:auto;">'      + '<table style="width:100%;border-collapse:collapse;">'      + '<thead><tr style="background:#F2EDE4;">'      + '<th style="width:28px;padding:8px 4px;border-right:1px solid #E8E0D0;"></th>'      + '<th style="text-align:left;padding:8px 12px;font-family:var(--font,sans-serif);font-size:12px;font-weight:700;letter-spacing:.06em;color:#7A5A30;border-right:1px solid #E8E0D0;">Atividade</th>'      + '<th style="width:70px;padding:8px 4px;font-family:var(--font,sans-serif);font-size:11px;font-weight:700;color:#9A8A6A;border-right:1px solid #E8E0D0;text-align:center;">Efetivo</th>'      + '<th style="width:60px;padding:8px 4px;font-family:var(--font,sans-serif);font-size:11px;font-weight:700;color:#9A8A6A;border-right:1px solid #E8E0D0;text-align:center;">Ordem</th>'      + '<th style="width:32px;"></th>'      + '</tr></thead>'      + '<tbody>'+rows+'</tbody>'      + '</table>'      + '</div>'      + '<div style="padding:10px 16px;border-top:1px solid #EEE4C8;display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">'      + '<button onclick="_poAdicionarTarefaDisc('+faseIdx+','+di+')" '      + 'style="height:30px;padding:0 14px;background:#F2EDE4;border:1px solid #D8C8A8;border-radius:5px;font-family:var(--font,sans-serif);font-size:10px;font-weight:700;color:#7A5A30;cursor:pointer;">+ Atividade</button>'      + '<button id="po-disc-save" onclick="document.getElementById(\'modal-po-disc\').remove();_poRenderModal('+faseIdx+');onCfgChange();gRender();" '      + 'style="height:30px;padding:0 20px;background:'+_discCors[1]+';border:none;border-radius:5px;font-family:var(--font,sans-serif);font-size:11px;font-weight:700;color:#fff;cursor:pointer;letter-spacing:.04em;">Salvar</button>'      + '</div>'      + '</div>';
...
function _poShowPop(faseIdx, rect) {
  ...
  var html = '<div style="display:flex;gap:6px;align-items:center;margin-bottom:14px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#1A2535;">'
    ...
    + '<input type="date" value="'+G.fmtISO(ini)+'" disabled style="flex:1;border:1px solid #E8E0D4;border-radius:4px;padding:5px 8px;font-size:12px;font-family:inherit;background:#F8F4EE;color:#A09080;">'    + '</label>'
    ...
    + '<input type="date" value="'+G.fmtISO(fim)+'" disabled style="flex:1;border:1px solid #E8E0D4;border-radius:4px;padding:5px 8px;font-size:12px;font-family:inherit;background:#F8F4EE;color:#A09080;">'    + '</label>'
    // Vínculo FI
    ...
    + '<button onclick="_poPopAdjust('+faseIdx+',-1)" style="width:38px;height:100%;border:none;border-right:1px solid #C8CDD8;background:#F4F6F8;cursor:pointer;font-size:14px;color:#5A6275;">−</button>'    + '<div id="po-pop-du" style="flex:1;display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:700;color:#1A2535;gap:6px;">'    + du+'<span style="font-size:11px;font-weight:400;color:#8A95A8;">dias úteis</span></div>'    + '<button onclick="_poPopAdjust('+faseIdx+',1)" style="width:38px;height:100%;border:none;border-left:1px solid #C8CDD8;background:#F4F6F8;cursor:pointer;font-size:14px;color:#5A6275;">+</button>'    + '</div>'    + '</div>'
    // Botão aplicar
...
function tecFornRenderModal() {
  ...
  if (forns.length === 0) {
    ...
    forns.forEach(function(f) {
      ...
      html += '<tr style="border-bottom:1px solid var(--border);">'
        ...
        + '<input type="text" value="' + f.nome.replace(/"/g,'&quot;') + '" onchange="tecFornRenomear(\'' + f.id + '\',this.value)" style="width:100%;border:1px solid var(--border);border-radius:4px;padding:4px 8px;font-size:11px;background:var(--bg-surface2);color:var(--txt);font-family:var(--body);">'        + '</td>'        + '<td style="padding:8px 6px;text-align:center;">'        + '<button onclick="tecFornRemove(\'' + f.id + '\')" style="background:none;border:none;cursor:pointer;color:#E57373;font-size:14px;" title="Remover">✕</button>'        + '</td></tr>';    });    html += '</tbody></table>';
  }
  ...
  var discOpts = DISCIPLINAS_TEC.map(function(d){
    return '<option value="' + d.id + '">' + d.id + ' — ' + d.label + '</option>';  }).join('');
...
function pfRenderTec(el) {
  ...
  theadCols.forEach(function(col, ci) {
    ...
    th.style.cssText = 'padding:8px 10px;text-align:' + (ci >= 3 && ci <= 6 ? 'center' : 'left') + ';font-size:9px;font-weight:700;text-transform:uppercase;color:var(--txt-muted);border-bottom:2px solid var(--border);';    th.textContent = col;    trHead.appendChild(th);  });  thead.appendChild(trHead);  tbl.appendChild(thead);
  ...
  nomeInp.style.cssText = 'width:100%;height:32px;border:1px solid var(--border);border-radius:4px;background:var(--bg-surface2);color:var(--txt);font-size:11px;padding:0 8px;box-sizing:border-box;font-family:var(--body);';  nomeDiv.appendChild(nomeLbl); nomeDiv.appendChild(nomeInp);  grid.appendChild(nomeDiv);

  // Botão adicionar
  var addBtn = document.createElement('button');
  addBtn.style.cssText = 'height:32px;padding:0 16px;background:var(--accent,#00AEDF);color:#fff;border:none;border-radius:4px;font-family:var(--font);font-size:10px;font-weight:700;cursor:pointer;white-space:nowrap;';  addBtn.textContent = '+ Adicionar';  addBtn.addEventListener('click', function() {
...
function tecFornShowPop(fornId, tecId, rect) {
  ...
  _pop.id = 'g-pop-el';
  _pop.style.cssText = 'position:fixed;background:#fff;border-radius:10px;z-index:3000;box-shadow:0 10px 36px rgba(0,0,0,.22);border:1px solid #DDE1E8;font-family:var(--body,sans-serif);padding:16px;width:260px;visibility:hidden;';  _pop.innerHTML = n;  document.body.appendChild(_pop);  requestAnimationFrame(function(){
...
function tecFornSetView(view) {
  ...
  if (btnSep) { btnSep.style.background = isUni ? 'var(--bg-surface2)' : 'var(--accent)'; btnSep.style.color = isUni ? 'var(--txt-muted)' : '#fff'; btnSep.style.border = isUni ? '1px solid var(--border)' : '1px solid var(--accent)'; }  if (btnUni) { btnUni.style.background = isUni ? 'var(--accent)' : 'var(--bg-surface2)'; btnUni.style.color = isUni ? '#fff' : 'var(--txt-muted)'; btnUni.style.border = isUni ? '1px solid var(--accent)' : '1px solid var(--border)'; }  onCfgChange(); salvarDados(); gRender();
}
...
function tecFornAbrirModal(fornId) {
  ...
  hdr.innerHTML = '<div style="width:10px;height:10px;border-radius:2px;background:'+colTec+';flex-shrink:0;margin-top:3px;"></div>'
    ...
    +'</div>'
    +'<button onclick="document.getElementById(\'modal-tec-forn\').remove()" style="background:none;border:none;cursor:pointer;color:var(--txt-muted);font-size:18px;line-height:1;padding:0;">✕</button>';  modal.appendChild(hdr);
  ...
  [{key:'nome',lbl:'Nome',ph:'Nome do contato'},{key:'tel',lbl:'Telefone',ph:'(11) 00000-0000'},{key:'email',lbl:'E-mail',ph:'email@empresa.com'}].forEach(function(field) {
    ...
    inp.style.cssText = 'width:100%;border:1px solid var(--border);border-radius:4px;padding:5px 8px;font-size:11px;background:var(--bg-surface2);color:var(--txt);box-sizing:border-box;font-family:var(--body);';    inp.addEventListener('change',(function(k){return function(){tecFornSetContato(f.id,k,this.value);f.contato=f.contato||{};f.contato[k]=this.value;};})(field.key));    w.appendChild(l); w.appendChild(inp); ctGrid.appendChild(w);  });  ctSec.appendChild(ctGrid);  body.appendChild(ctSec);
  ...
  function renderEtapas() {
    ...
    tecIds.forEach(function(tecId, ti) {
      ...
      if (isKO) {
        ...
        iniV.style.cssText = 'border:1px solid var(--border);border-radius:4px;padding:3px 7px;font-size:11px;background:var(--bg-surface2);color:var(--txt);display:flex;flex-direction:column;gap:4px;';        iniV.appendChild(iniI);        iniV.appendChild(koToggleBtn);
      } else {
  ...
  btnClose.style.cssText = 'height:32px;padding:0 20px;background:var(--accent,#00AEDF);color:#fff;border:none;border-radius:6px;font-family:var(--font);font-size:10px;font-weight:700;cursor:pointer;letter-spacing:.06em;text-transform:uppercase;';  btnClose.addEventListener('click',function(){overlay.remove();});  foot.appendChild(btnClose);  modal.appendChild(foot);
...
function tecFornRenderUnificado(projFase, _r, _i, _o, _e, forns) {
  ...
  if (globalSpan && globalSpan.start && globalSpan.end) {
    ...
    var _tb = '<button onclick="tecFornToggleAll()" style="background:none;border:1px solid #C8D4D8;border-radius:3px;cursor:pointer;font-size:8px;padding:0 3px;color:#6A7A8A;font-weight:700;line-height:14px;margin-left:2px;">'+(allExpanded?'\u25b2\u25b2':'\u25bc\u25bc')+'</button>';    if (_r.length > 0) { var _ls=_r.length-1; var _old=_r[_ls]; var _cut=_old.lastIndexOf('</div>'); if(_cut>=0) _r[_ls]=_old.slice(0,_cut)+_tb+'</div>'+_old.slice(_cut+6); }
  }
  ...
  forns.forEach(function(forn) {
    ...
    // Header
    _r.push('<div class="gn1" style="height:'+ROW_H+'px;background:'+bgH+';display:flex;align-items:center;padding:0 5px 0 20px;border-bottom:1px solid '+bdH+';border-right:2px solid #C0C8D4;gap:4px;">'      +'<span style="width:4px;height:4px;border-radius:1px;background:'+colTec+';flex-shrink:0;"></span>'      +'<span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-weight:700;color:'+colTec+';font-size:9px;">'+forn.nome+'</span>'      +'<span style="font-size:8px;color:#6A9A7A;flex-shrink:0;margin-right:2px;">'+forn.disciplinaId+'</span>'      +'<button data-fid="'+forn.id+'" onclick="tecFornAbrirModal(this.dataset.fid)" style="background:none;border:1px solid #C8D4D8;border-radius:3px;cursor:pointer;font-size:8px;padding:0 5px;color:#6A7A8A;font-weight:700;line-height:14px;">\u270e</button>'      +'</div>');
...
function _tpoRenderModal(ov) {
  ...
  let listHtml = todos.map((tpl, idx) => {
    ...
    return '<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--border);">'
      ...
      + '</div>'
      + '<button onclick="_tpoEditarTemplate(\'' + tpl.id + '\')" style="height:28px;padding:0 10px;background:var(--bg-surface2);border:1px solid var(--border);border-radius:5px;font-size:10px;font-family:var(--font);font-weight:700;color:var(--txt);cursor:pointer;">Editar</button>'      + (!isDefault ? '<button onclick="_tpoExcluirTemplate(\'' + tpl.id + '\')" style="height:28px;padding:0 10px;background:rgba(180,20,20,.08);border:1px solid rgba(180,20,20,.2);border-radius:5px;font-size:10px;font-family:var(--font);font-weight:700;color:#B41414;cursor:pointer;">Excluir</button>' : '')      + '</div>';  }).join('');

  ov.innerHTML = '<div style="background:var(--bg-panel);border-radius:12px;box-shadow:0 24px 64px rgba(0,0,0,.45);width:100%;max-width:640px;max-height:88vh;display:flex;flex-direction:column;overflow:hidden;">'    + '<div style="display:flex;align-items:center;gap:10px;padding:16px 20px;border-bottom:1px solid var(--border);flex-shrink:0;">'    + '<span style="font-family:var(--font);font-size:13px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:var(--txt);flex:1;">Templates de Pré-Obra</span>'    + '<button onclick="_tpoNovoTemplate()" style="height:30px;padding:0 14px;background:var(--accent);color:#0D1117;border:none;border-radius:5px;font-family:var(--font);font-size:10px;font-weight:700;cursor:pointer;">+ Novo</button>'    + '<button onclick="document.getElementById(\'modal-tpo-overlay\').remove()" style="width:30px;height:30px;background:var(--bg-surface2);border:1px solid var(--border);border-radius:5px;cursor:pointer;font-size:14px;color:var(--txt-muted);">✕</button>'    + '</div>'    + '<div style="flex:1;overflow-y:auto;padding:0 20px;">' + listHtml + '</div>'    + '</div>';
}
  ...
  function render() {
    let discsHtml = (tpl.disciplinas || []).map((disc, di) => {
      let tasksHtml = (disc.tasks || []).map((task, ti) =>
        ...
        + '<svg width="10" height="14" viewBox="0 0 8 12" style="opacity:.3;flex-shrink:0;cursor:grab;"><circle cx="2" cy="2" r="1.2" fill="currentColor"/><circle cx="6" cy="2" r="1.2" fill="currentColor"/><circle cx="2" cy="6" r="1.2" fill="currentColor"/><circle cx="6" cy="6" r="1.2" fill="currentColor"/><circle cx="2" cy="10" r="1.2" fill="currentColor"/><circle cx="6" cy="10" r="1.2" fill="currentColor"/></svg>'        + '<input type="text" value="' + task.n.replace(/"/g,'&quot;') + '" onchange="tpl.disciplinas[' + di + '].tasks[' + ti + '].n=this.value" style="flex:1;height:26px;padding:0 6px;border:0.5px solid var(--border);border-radius:4px;background:var(--bg-surface2);color:var(--txt);font-size:11px;">'        + '<label style="font-size:10px;color:var(--txt-muted);white-space:nowrap;">Efetivo</label>'        + '<input type="number" min="1" max="50" value="' + (task.prof||2) + '" onchange="tpl.disciplinas[' + di + '].tasks[' + ti + '].prof=parseInt(this.value)||1" style="width:44px;height:26px;padding:0 4px;border:0.5px solid var(--border);border-radius:4px;background:var(--bg-surface2);color:var(--txt);font-size:11px;text-align:center;">'        + (!isDefault ? '<button onclick="tpl.disciplinas[' + di + '].tasks.splice(' + ti + ',1);render()" style="width:22px;height:22px;background:none;border:none;cursor:pointer;font-size:12px;color:var(--txt-dim);" title="Remover tarefa">✕</button>' : '')        + '</div>'      ).join('');
      ...
      return '<div style="border:1px solid var(--border);border-radius:8px;margin-bottom:10px;overflow:hidden;">'
        ...
        + '<input type="text" value="' + disc.label.replace(/"/g,'&quot;') + '" onchange="tpl.disciplinas[' + di + '].label=this.value" style="flex:1;height:28px;padding:0 8px;border:0.5px solid var(--border);border-radius:4px;background:var(--bg-surface);color:var(--txt);font-size:12px;font-weight:700;">'        + '<label style="display:flex;align-items:center;gap:4px;font-size:11px;color:var(--txt-muted);cursor:pointer;"><input type="checkbox" ' + (disc.ativo!==false?'checked':'') + ' onchange="tpl.disciplinas[' + di + '].ativo=this.checked" style="accent-color:var(--accent);"> Ativa</label>'        + (!isDefault ? '<button onclick="tpl.disciplinas.splice(' + di + ',1);render()" style="height:26px;padding:0 8px;background:rgba(180,20,20,.08);border:1px solid rgba(180,20,20,.2);border-radius:4px;font-size:10px;color:#B41414;cursor:pointer;">✕ Disc.</button>' : '')        + '</div>'        + '<div style="padding:8px 10px;">' + tasksHtml        + (!isDefault ? '<button onclick="tpl.disciplinas[' + di + '].tasks.push({n:\'Nova tarefa\',prof:2});render()" style="margin-top:4px;font-size:10px;color:var(--accent);background:none;border:none;cursor:pointer;font-family:var(--font);font-weight:700;">+ Tarefa</button>' : '')        + '</div>'        + '</div>';    }).join('');

    ov.innerHTML = '<div style="background:var(--bg-panel);border-radius:12px;box-shadow:0 24px 64px rgba(0,0,0,.45);width:100%;max-width:660px;max-height:88vh;display:flex;flex-direction:column;overflow:hidden;">'      + '<div style="display:flex;align-items:center;gap:10px;padding:14px 20px;border-bottom:1px solid var(--border);flex-shrink:0;">'      + '<button onclick="_tpoRenderModal(document.getElementById(\'modal-tpo-overlay\'))" style="height:28px;padding:0 10px;background:var(--bg-surface2);border:1px solid var(--border);border-radius:5px;font-size:11px;color:var(--txt-muted);cursor:pointer;">← Voltar</button>'      + '<input type="text" id="tpo-edit-label" value="' + tpl.label.replace(/"/g,'&quot;') + '" onchange="tpl.label=this.value" style="flex:1;height:32px;padding:0 10px;border:1px solid var(--border);border-radius:6px;background:var(--bg-surface2);color:var(--txt);font-size:14px;font-weight:700;">'      + '<button onclick="document.getElementById(\'modal-tpo-overlay\').remove()" style="width:30px;height:30px;background:var(--bg-surface2);border:1px solid var(--border);border-radius:5px;cursor:pointer;font-size:14px;color:var(--txt-muted);">✕</button>'      + '</div>'      + '<div style="flex:1;overflow-y:auto;padding:16px 20px;">' + discsHtml      + (!isDefault ? '<button onclick="tpl.disciplinas.push({id:\'disc-\'+Date.now(),label:\'Nova Disciplina\',ativo:true,tasks:[{n:\'Tarefa\',prof:2}]});render()" style="width:100%;height:32px;background:var(--bg-surface2);border:1px dashed var(--border);border-radius:6px;font-family:var(--font);font-size:10px;font-weight:700;color:var(--txt-muted);cursor:pointer;">+ Adicionar Disciplina</button>' : '')      + '</div>'      + (!isDefault ? '<div style="padding:12px 20px;border-top:1px solid var(--border);display:flex;justify-content:flex-end;gap:8px;">'      + '<button onclick="_tpoRenderModal(document.getElementById(\'modal-tpo-overlay\'))" style="height:32px;padding:0 16px;background:var(--bg-surface2);border:1px solid var(--border);border-radius:6px;font-family:var(--font);font-size:10px;font-weight:700;color:var(--txt-muted);cursor:pointer;">Cancelar</button>'      + '<button onclick="_tpoSalvarEdicao(tpl)" style="height:32px;padding:0 20px;background:var(--accent);border:none;border-radius:6px;font-family:var(--font);font-size:10px;font-weight:700;color:#0D1117;cursor:pointer;">✓ Salvar Template</button>'      + '</div>' : '')      + '</div>';
  }
  render();
}
  ...
  ov.innerHTML = `
    <div style="background:var(--bg-panel);border-radius:12px;box-shadow:0 24px 64px rgba(0,0,0,.45);width:100%;max-width:640px;max-height:88vh;display:flex;flex-direction:column;overflow:hidden;">
      <div style="display:flex;align-items:center;gap:10px;padding:16px 20px;border-bottom:1px solid var(--border);flex-shrink:0;">
        ...
        <button onclick="tplNovo()" style="height:30px;padding:0 14px;background:var(--accent);color:#fff;border:none;border-radius:5px;font-family:var(--font);font-size:10px;font-weight:700;cursor:pointer;letter-spacing:.04em;">+ Novo</button>        <button onclick="document.getElementById('modal-tpl-overlay').remove()" style="width:28px;height:28px;border:1px solid var(--border);background:var(--bg-surface2);border-radius:6px;cursor:pointer;font-size:15px;color:var(--txt-muted);">✕</button>      </div>      <div id="tpl-lista" style="flex:1;overflow-y:auto;padding:14px;display:flex;flex-direction:column;gap:8px;"></div>    </div>`;  document.body.appendChild(ov);  tplRenderLista();
}
...
function tplRenderLista() {
  ...
  el.innerHTML = todos.map(tpl => {
    ...
    return `<div style="display:flex;align-items:center;gap:12px;padding:12px 14px;background:var(--bg-surface);border:1px solid var(--border);border-radius:8px;">
      <div style="width:38px;height:38px;border-radius:8px;background:${isCustom?'rgba(138,58,168,.12)':'rgba(0,174,223,.10)'};display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;">${emoji}</div>      <div style="flex:1;min-width:0;">        <div style="display:flex;align-items:center;gap:7px;margin-bottom:3px;">          <span style="font-family:var(--font);font-size:11px;font-weight:700;color:var(--txt);">${nome}</span>          <span style="font-size:8px;font-weight:700;padding:1px 6px;border-radius:8px;${isCustom?'background:rgba(138,58,168,.12);color:#7A3AA8;':'background:rgba(0,174,223,.10);color:var(--accent);'}">${isCustom?'Personalizado':'Padrão A|W'}</span>        </div>        <div style="font-size:10px;color:var(--txt-muted);">${nAtivas} disciplinas ativas · ${nTarefas} tarefas${tpl.desc?' · '+tpl.desc:''}</div>      </div>      <div style="display:flex;gap:5px;flex-shrink:0;">        <button onclick="tplAbrirEditor('${tpl.id}')" style="height:28px;padding:0 12px;background:var(--accent);color:#fff;border:none;border-radius:5px;font-family:var(--font);font-size:9px;font-weight:700;cursor:pointer;">✎ Editar</button>        <button onclick="tplDuplicar('${tpl.id}')" style="height:28px;padding:0 9px;border:1px solid var(--border);background:var(--bg-surface2);color:var(--txt-muted);border-radius:5px;font-family:var(--font);font-size:9px;font-weight:700;cursor:pointer;" title="Duplicar template">⧉</button>        ${isOriginalPadrao ? '' : `<button onclick="tplExcluir('${tpl.id}')" style="height:28px;padding:0 9px;border:1px solid rgba(184,52,24,.3);background:rgba(184,52,24,.06);color:var(--red,#B83418);border-radius:5px;font-family:var(--font);font-size:9px;font-weight:700;cursor:pointer;" title="Excluir">🗑</button>`}      </div>    </div>`;  }).join('');
}
        ...
        <div style="flex:1;">
          ...
          <div style="display:flex;align-items:center;gap:8px;">
            <input id="tpl-ed-nome" type="text" value="${nomeLabel}" placeholder="Nome do template"
              style="font-family:var(--font);font-size:13px;font-weight:600;color:#fff;background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.3);border-radius:5px;outline:none;padding:4px 10px;min-width:280px;"              onfocus="this.style.background='rgba(255,255,255,.25)'" onblur="this.style.background='rgba(255,255,255,.15)'">            <input id="tpl-ed-desc" type="text" value="${tpl.desc||''}" placeholder="Descrição opcional"              style="font-family:var(--body);font-size:11px;color:rgba(255,255,255,.8);background:rgba(255,255,255,.10);border:1px solid rgba(255,255,255,.2);border-radius:5px;outline:none;padding:4px 10px;flex:1;">          </div>        </div>        <div style="display:flex;gap:5px;flex-shrink:0;">          <button onclick="tplEdExpandirTudo(false)" title="Recolher todas as disciplinas" style="height:36px;padding:0 12px;background:rgba(255,255,255,.15);color:#fff;border:1px solid rgba(255,255,255,.25);border-radius:6px;font-family:var(--font);font-size:10px;font-weight:700;cursor:pointer;">⊟ Recolher</button>          <button onclick="tplEdExpandirTudo(true)" title="Expandir todas as disciplinas" style="height:36px;padding:0 12px;background:rgba(255,255,255,.15);color:#fff;border:1px solid rgba(255,255,255,.25);border-radius:6px;font-family:var(--font);font-size:10px;font-weight:700;cursor:pointer;">⊞ Expandir</button>        </div>        <button onclick="_tplSalvar()" style="height:36px;padding:0 20px;background:#fff;color:var(--accent);border:none;border-radius:6px;font-family:var(--font);font-size:11px;font-weight:700;cursor:pointer;letter-spacing:.04em;flex-shrink:0;">💾 Salvar</button>        <button onclick="document.getElementById('modal-tpl-editor').remove()" style="width:32px;height:32px;background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.25);border-radius:6px;cursor:pointer;font-size:16px;color:#fff;flex-shrink:0;">✕</button>      </div>      <!-- Conteúdo rolável -->      <div id="tpl-ed-content" style="flex:1;overflow-y:auto;padding:0;"></div>      <!-- Footer -->      <div style="padding:10px 18px;border-top:1px solid var(--border);flex-shrink:0;display:flex;align-items:center;justify-content:space-between;background:var(--bg-surface);">        <span style="font-size:10px;color:var(--txt-dim);">Todas as alterações são aplicadas ao clicar em 💾 Salvar</span>        <button onclick="_tplSalvar()" style="height:32px;padding:0 20px;background:var(--accent);color:#fff;border:none;border-radius:5px;font-family:var(--font);font-size:10px;font-weight:700;cursor:pointer;">💾 Salvar template</button>      </div>    </div>`;
...
function tplEdRender() {
  ...
  discs.forEach((disc, di) => {
    ...
    html += `<tr style="cursor:pointer;background:${ativa ? cor : '#8A8A8A'};" onclick="tplEdToggleDisc(${di})" draggable="true" ondragstart="tplEdDragStart(event,${di})" ondragover="tplEdDragOver(event,${di})" ondrop="tplEdDrop(event,${di})" ondragend="tplEdDragEnd()" id="tpl-disc-row-${di}">`;    html += `<td colspan="12" style="padding:8px 10px;border:1px solid var(--border);font-family:var(--font);font-size:12px;font-weight:700;color:#fff;">`;    html += `<span onclick="event.stopPropagation()" style="margin-right:10px;cursor:grab;opacity:.7;font-size:15px;letter-spacing:-1px;" title="Arrastar para reordenar">⠿⠿</span>`;    html += `<span style="margin-right:8px;">${isExp ? '▼' : '▶'}</span>`;
    ...
    (disc.tasks || []).forEach((task, ti) => {
      ...
      // Nome
      html += `<td style="padding:4px 4px 4px 20px;border:1px solid var(--border);">`;
      html += `<input type="text" value="${(task.n||'').replace(/"/g,'\"')}" onchange="tplEdSetNome(${di},${ti},this.value)" style="width:100%;border:none;background:transparent;font-size:11px;font-family:var(--body);">`;      html += `</td>`;
      // Tipo
      html += `<td style="padding:3px;border:1px solid var(--border);text-align:center;">`;
      ...
      // Efetivo
      html += `<td style="padding:3px;border:1px solid var(--border);text-align:center;">`;
      html += `<input type="number" min="0" max="99" value="${task.prof||0}" onchange="tplEdSetProf(${di},${ti},this.value)" style="width:44px;border:1px solid var(--border);border-radius:3px;text-align:center;font-size:11px;padding:2px 0;">`;      html += `</td>`;
      // M1-M8
      for (let m = 1; m <= 8; m++) {
        const perc = task.m[m] || 0;
        const mbg = perc > 0 ? `background:${corBg};` : '';
      ...
      // Remover tarefa
      html += `<td style="padding:3px;border:1px solid var(--border);text-align:center;">`;
      html += `<button onclick="tplEdRemoverTarefa(${di},${ti})" style="background:none;border:1px solid #E08080;color:#C04040;border-radius:3px;padding:2px 5px;font-size:11px;cursor:pointer;">✕</button>`;      html += `</td></tr>`;    });  });
  ...
  html += `<button onclick="tplEdAdicionarDisc()" style="background:var(--accent);color:#fff;border:none;padding:7px 18px;border-radius:5px;font-size:11px;cursor:pointer;font-weight:600;">+ Nova Disciplina</button>`;  html += `</td></tr>`;  html += '</tbody></table>';
...
function renderDisciplinasCompiladas() {
  ...
  DISC_DEFS.forEach((disc, discIdx) => {
    ...
    html += `${disc.label}`;
    html += `<button onclick="event.stopPropagation();removerDisciplina('${disc.id}')" style="float:right;background:rgba(255,255,255,0.2);border:1px solid rgba(255,255,255,0.3);color:#fff;padding:2px 6px;border-radius:3px;font-size:10px;cursor:pointer;margin-left:10px;">🗑 Remover</button>`;    html += `<button onclick="event.stopPropagation();adicionarTarefa('${disc.id}')" style="float:right;background:rgba(255,255,255,0.2);border:1px solid rgba(255,255,255,0.3);color:#fff;padding:2px 6px;border-radius:3px;font-size:10px;cursor:pointer;">+ Tarefa</button>`;    html += '</td>';    html += '</tr>';
    ...
    if (isExpanded) {
      disc.tasks.forEach((task, taskIdx) => {
        ...
        html += `<input type="number" value="${task.prof || 0}" min="0" max="10" onchange="atualizarProfTarefa('${disc.id}', ${taskIdx}, this.value)" style="width:100%;border:none;background:transparent;text-align:center;font-size:10px;">`;        html += `</td>`;
        ...
        // Total com validação
        html += `<td style="padding:8px;border:1px solid var(--border);text-align:center;background:${isIncomplete ? 'rgba(255,200,0,0.2)' : total === 100 ? 'rgba(0,200,0,0.1)' : 'transparent'};color:${isIncomplete ? '#e67e00' : total === 100 ? '#00a000' : '#666'};">`;        html += `<strong>${total}%</strong>`;        if (isIncomplete) html += ` ⚠️`;        html += `</td>`;
        
        // Ações
        html += `<td style="padding:8px;border:1px solid var(--border);text-align:center;">`;
        html += `<button onclick="removerTarefa('${disc.id}', ${taskIdx})" style="background:#ff4757;color:#fff;border:none;padding:3px 6px;border-radius:3px;font-size:10px;cursor:pointer;">🗑</button>`;        html += `</td>`;        html += '</tr>';      });
    }  });
  ...
  html += '<button onclick="adicionarNovaDisciplina()" style="background:var(--accent);color:#fff;border:none;padding:8px 16px;border-radius:5px;font-size:11px;cursor:pointer;font-weight:600;">+ Nova Disciplina</button>';  html += '</td>';  html += '</tr>';
...
function _pfRender() {
  ...
  const arquitetosHTML = _pfDados.arquitetos.map((arq,i) => `
    <div class="pf-arq-card" draggable="true" ondragstart="_pfDragStart(${i},event)" ondragend="_pfDragEnd(event)"
      style="display:flex;align-items:center;gap:7px;padding:7px 8px;border-radius:7px;border:0.5px solid var(--border);cursor:grab;background:var(--bg-panel);margin-bottom:4px;transition:opacity .15s;">      <svg width="8" height="12" viewBox="0 0 8 12" fill="none" style="opacity:.3;flex-shrink:0;color:var(--txt-muted);">        <circle cx="2" cy="2" r="1.2" fill="currentColor"/><circle cx="6" cy="2" r="1.2" fill="currentColor"/>        <circle cx="2" cy="6" r="1.2" fill="currentColor"/><circle cx="6" cy="6" r="1.2" fill="currentColor"/>        <circle cx="2" cy="10" r="1.2" fill="currentColor"/><circle cx="6" cy="10" r="1.2" fill="currentColor"/>      </svg>      <div style="width:28px;height:28px;border-radius:50%;background:${arq.cor}22;border:1.5px solid ${arq.cor};display:flex;align-items:center;justify-content:center;font-family:var(--font);font-size:10px;font-weight:700;color:${arq.cor};flex-shrink:0;">${_pfIni2(arq.nome)}</div>      <div style="min-width:0;">        <div style="font-size:12px;font-weight:600;color:var(--txt);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${arq.nome}</div>        <div style="font-size:10px;color:var(--txt-dim);">${arq.cargo||''}</div>      </div>    </div>`).join('');
  ...
  const col1 = `
    <div style="border-right:0.5px solid var(--border);display:flex;flex-direction:column;overflow:hidden;">
      <div style="padding:8px 12px;font-family:var(--font);font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--txt-muted);background:var(--bg-surface);border-bottom:0.5px solid var(--border);">Arquitetos</div>      <div style="padding:8px 10px;flex:1;overflow-y:auto;">        <p style="font-size:10px;color:var(--txt-dim);margin-bottom:8px;">Arraste para a etapa →</p>        ${arquitetosHTML}        <button onclick="_pfAddArquiteto()" style="margin-top:6px;font-size:11px;color:var(--txt-dim);background:none;border:none;cursor:pointer;padding:4px 2px;">+ novo arquiteto</button>        ${todasDropZone}      </div>    </div>`;
  ...
  subIds.forEach((subId,idx) => {
    ...
    const chips = arqsNaEtapa.map(arq =>
      `<div onclick="event.stopPropagation();_pfRemArqEtapa('${subId}','${arq.id}')" title="Clique para remover ${arq.nome}"
        style="width:24px;height:24px;border-radius:50%;background:${arq.cor}22;border:1.5px solid ${arq.cor};display:flex;align-items:center;justify-content:center;font-family:var(--font);font-size:9px;font-weight:700;color:${arq.cor};cursor:pointer;flex-shrink:0;transition:opacity .12s;" onmouseover="this.style.opacity='.5'" onmouseout="this.style.opacity='1'">${_pfIni2(arq.nome)}</div>`    ).join('');
  ...
  const col2 = `
    <div style="border-right:0.5px solid var(--border);display:flex;flex-direction:column;overflow:hidden;min-width:0;">
      <div style="padding:8px 12px;font-family:var(--font);font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--txt-muted);background:var(--bg-surface);border-bottom:0.5px solid var(--border);">Etapas</div>      <div style="flex:1;overflow-y:auto;">${etapasHTML}</div>    </div>`;
  ...
  if (_pfEtapaSel) {
    ...
    col3Inner = `
      <div style="padding:8px 12px;font-family:var(--font);font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:${etCor};background:${etCor}0A;border-bottom:0.5px solid var(--border);">${nomeSel} · tarefas</div>      <div style="padding:10px 12px;flex:1;overflow-y:auto;">        ${tarefasList}        <div style="margin-top:8px;border-top:0.5px solid var(--border);padding-top:8px;display:flex;gap:6px;">          <input id="pf-nova-tarefa" type="text" placeholder="Nova tarefa..." onkeydown="if(event.key==='Enter')_pfAddTarefa()"            style="flex:1;height:30px;padding:0 10px;border:0.5px solid var(--border);border-radius:6px;background:var(--bg-surface);color:var(--txt);font-size:11px;outline:none;">          <button onclick="_pfAddTarefa()" style="height:30px;padding:0 12px;background:var(--accent);border:none;border-radius:6px;font-family:var(--font);font-size:9px;font-weight:700;color:#0D1117;cursor:pointer;">Ok</button>        </div>      </div>`;
  } else {
    col3Inner = `
      <div style="padding:8px 12px;font-family:var(--font);font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--txt-muted);background:var(--bg-surface);border-bottom:0.5px solid var(--border);">Tarefas</div>      <div style="flex:1;display:flex;align-items:center;justify-content:center;padding:24px;text-align:center;">        <p style="font-size:12px;color:var(--txt-dim);line-height:1.6;">Selecione uma etapa<br>para ver as tarefas</p>      </div>`;
  }
  ...
  const legenda = subIds.map((subId,idx) => {
    ...
    return `<div onclick="_pfToggleDestaque('${subId}')" style="display:flex;align-items:center;gap:5px;margin-bottom:4px;cursor:pointer;padding:2px 4px;border-radius:4px;background:${isAtivo?etCor+'18':'transparent'};border:0.5px solid ${isAtivo?etCor:'transparent'};">      <div style="width:10px;height:10px;border-radius:2px;background:${etCor};flex-shrink:0;"></div>      <span style="font-size:10px;color:${isAtivo?etCor:'var(--txt-muted)'};">${subNames[subId]||subId}</span>    </div>`;  }).join('');

  const col4 = `
    <div style="display:flex;flex-direction:column;overflow:hidden;">
      <div style="padding:8px 12px;font-family:var(--font);font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--txt-muted);background:var(--bg-surface);border-bottom:0.5px solid var(--border);display:flex;justify-content:space-between;align-items:center;">        <span>Calendário</span>        <div style="display:flex;gap:4px;">          <button onclick="_pfCalNav(-1)" style="width:20px;height:20px;background:var(--bg-surface2);border:0.5px solid var(--border);border-radius:4px;cursor:pointer;font-size:12px;color:var(--txt-muted);padding:0;">‹</button>          <button onclick="_pfCalNav(1)" style="width:20px;height:20px;background:var(--bg-surface2);border:0.5px solid var(--border);border-radius:4px;cursor:pointer;font-size:12px;color:var(--txt-muted);padding:0;">›</button>        </div>      </div>      <div style="padding:10px 12px;flex:1;overflow-y:auto;">        ${renderMes(ano,mes)}        ${renderMes(ano2,mes2)}        <div style="padding-top:8px;border-top:0.5px solid var(--border);">          <p style="font-size:10px;color:var(--txt-dim);margin-bottom:6px;">Clique para destacar</p>          ${legenda}        </div>      </div>    </div>`;
...
function _pfResumo(){
  ...
  d.innerHTML=`<div style="background:#fff;border-radius:10px;width:100%;max-width:820px;max-height:90vh;display:flex;flex-direction:column;overflow:hidden;">
    <div style="display:flex;align-items:center;padding:14px 20px;border-bottom:1px solid #E8ECF0;background:#1A2535;">
      ...
      <button onclick="this.closest('[style*=fixed]').remove()" style="width:28px;height:28px;background:rgba(255,255,255,.1);border:1px solid rgba(255,255,255,.2);border-radius:5px;cursor:pointer;font-size:13px;color:rgba(255,255,255,.6);margin-left:14px;">✕</button>    </div>    <div style="flex:1;overflow:auto;">      <table style="width:100%;border-collapse:collapse;">        <thead><tr style="background:#F4F6F8;">          <th style="padding:8px 10px;text-align:left;font-family:Oswald,sans-serif;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:#8A95A8;border-bottom:1px solid #E8ECF0;width:160px;">Etapa</th>          <th style="padding:8px 10px;text-align:left;font-family:Oswald,sans-serif;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:#8A95A8;border-bottom:1px solid #E8ECF0;width:170px;">Responsáveis</th>          <th style="padding:8px 10px;text-align:left;font-family:Oswald,sans-serif;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:#8A95A8;border-bottom:1px solid #E8ECF0;">Tarefas</th>        </tr></thead>        <tbody>${rows}</tbody>      </table>    </div>    <div style="padding:12px 20px;border-top:1px solid #E8ECF0;display:flex;justify-content:space-between;align-items:center;">      <span style="font-size:11px;color:#9AA0AF;">Gerado em ${new Date().toLocaleDateString('pt-BR')}</span>      <div style="display:flex;gap:8px;">        <button onclick="this.closest('[style*=fixed]').remove()" style="height:32px;padding:0 16px;background:#F4F6F8;border:1px solid #D8DCE4;border-radius:6px;font-family:Oswald,sans-serif;font-size:10px;font-weight:700;color:#5A6275;cursor:pointer;">Fechar</button>        <button onclick="window.print()" style="height:32px;padding:0 20px;background:#00DEDB;border:none;border-radius:6px;font-family:Oswald,sans-serif;font-size:10px;font-weight:700;color:#0D1117;cursor:pointer;">🖨 Imprimir A4</button>      </div>    </div>  </div>`;  document.body.appendChild(d);
}
...
function gBuildAprovaHtml(t){
  ...
  el.innerHTML=[
    ...
    "<input type=\"checkbox\" id=\"pop-aprova-chk\""+chkd+" onchange=\"gPopAprovaChange(this.parentElement.parentElement.dataset.aprovaSid)\" style=\"accent-color:#D4A017;width:13px;height:13px;cursor:pointer;\">",    "<span style=\"font-size:12px;color:#3A4A5A;\">Requer aprovação do cliente</span></label>",    "<label style=\"display:flex;align-items:center;gap:7px;\">",    "<span style=\"font-size:11px;font-weight:700;color:#8A95A8;width:36px;\">Dias</span>",    "<input type=\"number\" id=\"pop-aprova-dias\" value=\""+ac.dias+"\" min=\"1\" max=\"10\" onchange=\"gPopAprovaChange(this.parentElement.parentElement.dataset.aprovaSid)\" style=\"width:50px;padding:4px 6px;border:1px solid #C8CDD8;border-radius:4px;font-size:13px;text-align:center;\" "+dis+">",    "</label>",    "<div style=\"margin-top:10px;border-top:1px solid #EEF0F4;padding-top:10px;display:flex;flex-direction:column;gap:7px;\">",    "<div style=\"font-size:9px;font-weight:700;text-transform:uppercase;color:#8A95A8;margin-bottom:2px;\">Aparência da marcação</div>",    "<label style=\"display:flex;align-items:center;gap:8px;\">",    "<span style=\"font-size:11px;color:#5A6A7A;min-width:60px;\">Fundo</span>",    "<input type=\"color\" id=\"pop-aprova-bg\" value=\""+ (_bc==='transparent'?'#e60a20':_bc) +"\" oninput=\"if(!ESTADO.cfg)ESTADO.cfg={};ESTADO.cfg.aprovaBgColor=this.value;document.getElementById('pop-aprova-bg-transp').checked=false;salvarDados();gRender()\" style=\"width:32px;height:24px;border:1px solid #C8CDD8;border-radius:4px;padding:1px;cursor:pointer;\">",    "<label style=\"display:flex;align-items:center;gap:4px;font-size:10px;color:#8A95A8;cursor:pointer;\">",    "<input type=\"checkbox\" id=\"pop-aprova-bg-transp\" "+(_bc==='transparent'?' checked':'')+"\" onchange=\"if(!ESTADO.cfg)ESTADO.cfg={};ESTADO.cfg.aprovaBgColor=this.checked?'transparent':document.getElementById('pop-aprova-bg').value;salvarDados();gRender()\" style=\"accent-color:#009EA8;cursor:pointer;\">",    "transparente</label>",    "</label>",    "<label style=\"display:flex;align-items:center;gap:8px;\">",    "<span style=\"font-size:11px;color:#5A6A7A;min-width:60px;\">Estrela ★</span>",    "<input type=\"color\" id=\"pop-aprova-star\" value=\""+_sc+"\" oninput=\"if(!ESTADO.cfg)ESTADO.cfg={};ESTADO.cfg.aprovaStarColor=this.value;salvarDados();gRender()\" style=\"width:32px;height:24px;border:1px solid #C8CDD8;border-radius:4px;padding:1px;cursor:pointer;\">",    "</label>",    "<div style=\"display:flex;align-items:center;gap:8px;\">",    "<span style=\"font-size:11px;color:#5A6A7A;min-width:60px;\">Preview</span>",    "<div id=\"pop-aprova-preview\" style=\"width:28px;height:18px;border-radius:3px;border:1px solid #C8CDD8;display:flex;align-items:center;justify-content:center;background:"+_bc+"\">",    "<span style=\"font-size:12px;color:"+_sc+"\">★</span></div>",    "</div>",    "</div>"  ].join("");
...
const PREOBRA_TEMPLATES_DEFAULT = [
  {id:'pre-obra-padrao',label:'🏗 Pré-Obra Padrão',desc:'Template padrão A|W',disciplinas:[
    {id:'demolicoes',label:'Demolições e Desmontagens',ativo:true,tasks:[{n:'Desmontagem de divisórias',prof:4},{n:'Demolição de alvenaria',prof:3},{n:'Remoção de revestimentos',prof:3},{n:'Descarte e limpeza',prof:2}]},    {id:'canteiro',label:'Montagem de Canteiro e Almoxarifado',ativo:true,tasks:[{n:'Delimitação do canteiro',prof:2},{n:'Montagem de almoxarifado',prof:3},{n:'Instalação de tapumes',prof:2},{n:'Organização de fluxo',prof:1}]},    {id:'comunicacao-visual',label:'Comunicação Visual de Obra',ativo:true,tasks:[{n:'Projeto de sinalização',prof:1},{n:'Instalação de banners',prof:2},{n:'Placas de segurança',prof:1},{n:'Proteção de áreas comuns',prof:2}]},    {id:'piso-elevado',label:'Montagem de Piso Elevado',ativo:true,tasks:[{n:'Nivelamento do substrato',prof:3},{n:'Instalação de pedestais',prof:4},{n:'Assentamento das placas',prof:4},{n:'Inspeção e ajustes',prof:2}]}  ]}];
...
function _preObraSalvarCustom(arr){try{localStorage.setItem('aw_preobra_templates',JSON.stringify(arr));}catch(e){}}
...
function _equipeChip(nome, role, sel, onclick) {
  ...
  return '<button onclick="'+onclick+'" style="display:flex;align-items:center;gap:5px;padding:5px 10px;border-radius:20px;border:1px solid '+brd+';background:'+bg+';color:'+clr+';font-size:11px;font-weight:700;font-family:var(--font);cursor:pointer;transition:all .15s;">'    + nome    + (role ? '<span style="font-size:9px;font-weight:400;opacity:.7;">'+role+'</span>' : '')    + '</button>';
}
...
function equipeRenderGerentes() {
  ...
  gLista.innerHTML = gerentes.map(function(g) {
    ...
    return _equipeChip(g.nome, 'GA', sel, "equipeToggleGerente('"+g.nome+"')");  }).join('');
  equipeRenderArqs();
}
...
function equipeRenderArqs() {
  ...
  aLista.innerHTML = todosArqs.map(function(a) {
    ...
    return _equipeChip(a.nome, a.cargo, sel, "equipeToggleArq('"+a.nome+"')");  }).join('');
...
window.abrirModalCongelar = function() {
  ...
  ov.innerHTML =
    ...
    +   '<button onclick="document.getElementById(\'modal-congelar\').remove()" style="width:28px;height:28px;border:1px solid var(--border);background:var(--bg-surface2);border-radius:6px;cursor:pointer;font-size:14px;color:var(--txt-muted);">✕</button>'    + '</div>'    + '<div style="flex:1;overflow-y:auto;padding:20px 22px;display:flex;flex-direction:column;gap:14px;">'    +   '<div style="display:flex;gap:11px;align-items:flex-start;"><span style="flex-shrink:0;width:22px;height:22px;border-radius:50%;background:rgba(0,185,80,.14);color:#00A048;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;font-family:var(--font);">1</span>'    +     '<p style="font-size:13px;line-height:1.6;color:var(--txt);margin:0;">A partir de agora todas as <strong>datas macro</strong> definidas neste cronograma estarão congeladas e serão monitoradas. As informações constantes aqui serão transferidas para o <strong>Comunicado Semanal</strong> e farão parte do contrato, devendo inclusive ser apresentadas no <strong>Kickoff Externo</strong>.</p></div>'    +   '<div style="display:flex;gap:11px;align-items:flex-start;"><span style="flex-shrink:0;width:22px;height:22px;border-radius:50%;background:rgba(0,185,80,.14);color:#00A048;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;font-family:var(--font);">2</span>'    +     '<p style="font-size:13px;line-height:1.6;color:var(--txt);margin:0;">Mudanças no planejamento deverão obedecer o <strong>protocolo de alteração</strong>, que incluirá revisão contratual e validação do cliente e da liderança da A|W.</p></div>'    +   '<div style="display:flex;gap:11px;align-items:flex-start;"><span style="flex-shrink:0;width:22px;height:22px;border-radius:50%;background:rgba(0,185,80,.14);color:#00A048;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;font-family:var(--font);">3</span>'    +     '<p style="font-size:13px;line-height:1.6;color:var(--txt);margin:0;">Os <strong>próximos passos</strong> incluem o planejamento fino da arquitetura, o refinamento do cronograma de projetos técnicos e o alinhamento dos prazos de execução dos serviços da planilha de venda.</p></div>'    + '</div>'    + '<div style="flex-shrink:0;padding:16px 22px;border-top:1px solid var(--border);display:flex;align-items:center;justify-content:flex-end;gap:10px;background:var(--bg-surface);">'    +   '<button onclick="document.getElementById(\'modal-congelar\').remove()" style="height:34px;padding:0 16px;border:1px solid var(--border);background:var(--bg-surface2);border-radius:7px;font-family:var(--font);font-size:10px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:var(--txt-muted);cursor:pointer;">Cancelar</button>'    +   '<button onclick="confirmarCongelamento()" style="height:34px;padding:0 20px;border:none;background:#00A048;border-radius:7px;font-family:var(--font);font-size:10px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#fff;cursor:pointer;display:flex;align-items:center;gap:6px;">❄ Concordo e congelar</button>'    + '</div></div>';  document.body.appendChild(ov);
};
...
function _abrirModalDescongelar() {
  ...
  ov.innerHTML =
    ...
    + '<p style="font-size:13px;line-height:1.6;color:var(--txt-muted);margin:0;">O cronograma voltará a ser editável e sairá do estado congelado. Use apenas para correções autorizadas — o protocolo de alteração se aplica.</p>'    + '<div style="display:flex;justify-content:flex-end;gap:10px;">'    +   '<button onclick="document.getElementById(\'modal-descongelar\').remove()" style="height:34px;padding:0 16px;border:1px solid var(--border);background:var(--bg-surface2);border-radius:7px;font-family:var(--font);font-size:10px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:var(--txt-muted);cursor:pointer;">Cancelar</button>'    +   '<button onclick="confirmarDescongelamento()" style="height:34px;padding:0 18px;border:none;background:var(--orange);border-radius:7px;font-family:var(--font);font-size:10px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#fff;cursor:pointer;">↺ Descongelar</button>'    + '</div></div>';  document.body.appendChild(ov);
}
