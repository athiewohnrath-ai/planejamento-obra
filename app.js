// Planejamento de Obra A|W — v5.11.16
...
function renderHistograma(){const t=document.getElementById("pane-histograma");t.innerHTML="";const e=gSt.obraFases[0];if(!e)return void(t.innerHTML='<div class="pane-placeholder"><div class="ph-title">Configure a obra primeiro</div></div>');const o=G.SEG_N,a=[3,5,7],dkFmt=t=>{const e=new Date(t);return e.getFullYear()+"-"+String(e.getMonth()+1).padStart(2,"0")+"-"+String(e.getDate()).padStart(2,"0")},n=new Map;gSt.obraFases.forEach(t=>{const{days:e}=gerarDiasModulos(t);e.forEach(t=>{if(t.buffer)return;const e=dkFmt(t.date);n.has(e)||n.set(e,{...t})})});// Calcular dias de pré-obra para todas as fases e adicionar ao r global
  ...
  t.totalByDate={};
  r.forEach(e=>{if(e.buffer||e.isSun)return;const o=dkFmt(e.date);t.totalByDate[o]=(t.disc.reduce((e,a,n)=>e+(t.profByDay[n][o]?.total||0),0))+(t.preObraByDay&&t.preObraByDay[o]||0)});
