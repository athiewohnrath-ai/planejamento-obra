// Planejamento de Obra A|W — v5.11.01
...
(function(obraFase, obraFaseIdx){
  ...
  var payload=encodeURIComponent(JSON.stringify({type:'preObra',faseIdx:obraFaseIdx}));
    // Sidebar header
  r.push('<div data-po-open="'+obraFaseIdx+'" class="gt2" style="height:'+(G.ROW_H+8)+'px;background:'+cb+';display:flex;align-items:center;padding:0 5px 0 8px;border-bottom:1px solid #EEE0C8;border-right:2px solid #C0C8D4;font-size:10px;font-weight:700;color:#6A3810;text-transform:uppercase;letter-spacing:.04em;gap:3px;cursor:pointer;"><span style="width:5px;height:5px;border-radius:1px;background:'+cm+';flex-shrink:0;"></span><span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">Pré-Obra'+(gSt.obraFases.length>1?' F'+(obraFaseIdx+1):'')+' · '+du+' DU</span><span style="font-size:9px;opacity:.5;"> ✏</span></div>');
...
function _poRenderModal(faseIdx) {
  ...
  var discsHtml = discs.map(function(disc, di) {
    var tasksHtml = (disc.tasks||[]).map(function(task, ti) {
      return '<div style="display:flex;align-items:center;gap:6px;padding:5px 8px 5px 32px;border-bottom:0.5px solid var(--border);">'
        ...
        + 'style="width:22px;height:22px;background:none;border:none;cursor:pointer;font-size:12px;color:var(--txt-dim);" title="Remover tarefa">✕</button>'
        + '</div>';
    }).join('');
    ...
    return '<div style="border:1px solid var(--border);border-radius:8px;margin-bottom:10px;overflow:hidden;">'
      ...
      + 'style="width:26px;height:26px;background:var(--bg-surface);border:1px solid var(--border);border-radius:4px;cursor:pointer;font-size:13px;">↑</button>'
  ...
  ov.innerHTML = '<div style="background:var(--bg-panel);border-radius:12px;box-shadow:0 24px 64px rgba(0,0,0,.45);width:100%;max-width:620px;max-height:88vh;display:flex;flex-direction:column;overflow:hidden;">'
...
function _poAbrirTarefas(faseIdx, di) {
  var existing = document.getElementById('modal-po-tasks');
  ...
  function renderTasks() {
    var ov = document.getElementById('modal-po-tasks');
    if (!ov) return;

    var tasksHtml = (disc.tasks||[]).map(function(task, ti) {
      return '<div style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:var(--bg-surface);border-radius:6px;border:0.5px solid var(--border);margin-bottom:6px;">'
        + '<div style="display:flex;flex-direction:column;gap:2px;flex:1;min-width:0;">'
        + '<input type="text" value="'+task.n.replace(/"/g,'&quot;')+'" '
        + 'onchange="ESTADO.preObraCustom['+faseIdx+'].disciplinas['+di+'].tasks['+ti+'].n=this.value;onCfgChange();" '
        + 'style="width:100%;height:28px;padding:0 8px;border:0.5px solid var(--border);border-radius:4px;background:var(--bg-surface2);color:var(--txt);font-size:12px;">'
        + '</div>'
        + '<label style="display:flex;align-items:center;gap:4px;font-size:11px;color:var(--txt-muted);white-space:nowrap;flex-shrink:0;">'
        + 'Efetivo'
        + '<input type="number" min="1" max="50" value="'+(task.prof||2)+'" '
        + 'onchange="ESTADO.preObraCustom['+faseIdx+'].disciplinas['+di+'].tasks['+ti+'].prof=parseInt(this.value)||1;onCfgChange();gRender();" '
        + 'style="width:48px;height:28px;padding:0 6px;border:0.5px solid var(--border);border-radius:4px;background:var(--bg-surface2);color:var(--txt);font-size:12px;text-align:center;"></label>'
        + '<button onclick="_poMoverTarefa('+faseIdx+','+di+','+ti+',-1)" '
        + 'style="width:26px;height:26px;background:var(--bg-surface2);border:1px solid var(--border);border-radius:4px;cursor:pointer;font-size:12px;" title="Mover para cima">↑</button>'
        + '<button onclick="_poMoverTarefa('+faseIdx+','+di+','+ti+',1)" '
        + 'style="width:26px;height:26px;background:var(--bg-surface2);border:1px solid var(--border);border-radius:4px;cursor:pointer;font-size:12px;" title="Mover para baixo">↓</button>'
        + '<button onclick="_poRemoverTarefaModal('+faseIdx+','+di+','+ti+')" '
        + 'style="width:26px;height:26px;background:rgba(180,20,20,.08);border:1px solid rgba(180,20,20,.2);border-radius:4px;cursor:pointer;font-size:12px;color:#B41414;">✕</button>'
        + '</div>';
    }).join('');

    ov.innerHTML = '<div style="background:var(--bg-panel);border-radius:10px;box-shadow:0 16px 48px rgba(0,0,0,.5);width:100%;max-width:520px;max-height:80vh;display:flex;flex-direction:column;overflow:hidden;">'
      + '<div style="display:flex;align-items:center;gap:10px;padding:12px 16px;border-bottom:1px solid var(--border);flex-shrink:0;background:var(--bg-surface2);">'
      + '<div style="width:5px;height:5px;border-radius:1px;background:#C07820;flex-shrink:0;"></div>'
      + '<div style="flex:1;">'
      + '<div style="font-family:var(--font);font-size:12px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:var(--txt);">'+disc.label+'</div>'
      + '<div style="font-size:10px;color:var(--txt-muted);">Tarefas da disciplina</div>'
      + '</div>'
      + '<button onclick="document.getElementById(\'modal-po-tasks\').remove();_poRenderModal('+faseIdx+');" '
      + 'style="width:28px;height:28px;background:var(--bg-surface);border:1px solid var(--border);border-radius:5px;cursor:pointer;font-size:13px;color:var(--txt-muted);">✕</button>'
      + '</div>'
      + '<div style="flex:1;overflow-y:auto;padding:12px 16px;">'
      + tasksHtml
      + '<button onclick="_poAdicionarTarefaModal('+faseIdx+','+di+')" '
      + 'style="width:100%;height:30px;background:var(--bg-surface2);border:1px dashed var(--border);border-radius:5px;font-family:var(--font);font-size:10px;font-weight:700;color:var(--txt-muted);cursor:pointer;margin-top:4px;">+ Tarefa</button>'
      + '</div>'
      + '<div style="padding:10px 16px;border-top:1px solid var(--border);display:flex;justify-content:flex-end;flex-shrink:0;">'
      + '<button onclick="document.getElementById(\'modal-po-tasks\').remove();_poRenderModal('+faseIdx+');gRender();" '
      + 'style="height:30px;padding:0 20px;background:var(--accent);border:none;border-radius:5px;font-family:var(--font);font-size:10px;font-weight:700;color:#0D1117;cursor:pointer;">✓ OK</button>'
      + '</div>'
      + '</div>';
  }

  var ov = document.createElement('div');
  ov.id = 'modal-po-tasks';
  ov.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.4);z-index:9600;display:flex;align-items:center;justify-content:center;padding:16px;';
  document.body.appendChild(ov);
  renderTasks();
  window._poRenderTasks = renderTasks;
}
window._poAbrirTarefas = _poAbrirTarefas;
...
function _poAdicionarTarefaModal(faseIdx, di) {
  ESTADO.preObraCustom[faseIdx].disciplinas[di].tasks.push({n:'Nova tarefa', prof:2});
  onCfgChange();
  if (window._poRenderTasks) window._poRenderTasks();
}
window._poAdicionarTarefaModal = _poAdicionarTarefaModal;

function _poRemoverTarefaModal(faseIdx, di, ti) {
  ESTADO.preObraCustom[faseIdx].disciplinas[di].tasks.splice(ti, 1);
  onCfgChange();
  if (window._poRenderTasks) window._poRenderTasks();
}
...
function gInitColResize() {
  ...
  handle.addEventListener('mousedown', function(e) {
    ...
    function onMove(e) {
      ...
      var newW = Math.max(120, Math.min(320, startW + dx));
      if (!ESTADO.cfg) ESTADO.cfg = {};
      ESTADO.cfg.lblW = newW;
      gRender();
    }
    function onUp() {
...
function gSetGanttFontSize(val) {
  val = Math.max(8, Math.min(14, parseInt(val) || 10));
...
function gGetGanttFontSize() {
  return (ESTADO && ESTADO.cfg && ESTADO.cfg.ganttFontSize) || 10;
}

function gSetFontetitulo(f) {
  document.documentElement.style.setProperty('--font', f);
  document.documentElement.style.setProperty('--font-titulo', f);
  if (!ESTADO.cfg) ESTADO.cfg = {};
  ESTADO.cfg.fonteTitulo = f;
  onCfgChange(); salvarDados();
}
function gSetFonteTexto(f) {
  document.documentElement.style.setProperty('--body', f);
  document.documentElement.style.setProperty('--cond', f);
  document.documentElement.style.setProperty('--font-texto', f);
  if (!ESTADO.cfg) ESTADO.cfg = {};
  ESTADO.cfg.fonteTexto = f;
  onCfgChange(); salvarDados();
}
window.gSetFontetitulo = gSetFontetitulo;
window.gSetFonteTexto = gSetFonteTexto;
