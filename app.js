// Planejamento de Obra A|W — v6.03.45
...
window.fecharGestaoArq = function(salvar) {
  ...
  // O iframe já salvou no Supabase via _autoSaveSB().
  // O pai só precisa atualizar o ESTADO em memória para a sessão atual.
  try {
    ...
    if (raw) {
      ...
      if (parsed.estado && parsed.estado.cfg && parsed.estado.cfg.equipeARQ) {
        ...
        // Atualiza o sessionStorage do pai com o estado completo (inclui equipeARQ)
        sessionStorage.setItem('aw_estado_atual', JSON.stringify({ts: Date.now(), estado: ESTADO}));
