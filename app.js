...
<th style="padding:5px 8px;text-align:center;font-family:var(--font);font-weight:700;letter-spacing:.05em;text-transform:uppercase;color:var(--txt-muted);border-bottom:1px solid var(--border);width:90px;">Ativa</th>
...
function gPopAprovaChange(subId){const chk=document.getElementById('pop-aprova-chk');const inp=document.getElementById('pop-aprova-dias');if(!chk||!inp)return;inp.disabled=!chk.checked;setAprovaConfig(subId,chk.checked,Math.max(1,parseInt(inp.value)||1));gRender();}
function ativSetAprova(idx,ativo){const list=_ativListas.arq;if(!list[idx])return;const subId=list[idx].id;setAprovaConfig(subId,ativo,getAprovaConfig(subId).dias);gRender();}