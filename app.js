...
(function(obraFase, obraFaseIdx){
  ...
  var payload=encodeURIComponent(JSON.stringify({type:'preObra',faseIdx:obraFaseIdx}));
    // Sidebar header
  r.push('<div data-po-open="'+obraFaseIdx+'" style="height:'+(G.ROW_H+8)+'px;background:'+cb+';display:flex;align-items:center;padding:0 5px 0 8px;border-bottom:1px solid #EEE0C8;border-right:2px solid #C0C8D4;font-size:10px;font-weight:700;color:#6A3810;text-transform:uppercase;letter-spacing:.04em;gap:3px;cursor:pointer;"><span style="width:5px;height:5px;border-radius:1px;background:'+cm+';flex-shrink:0;"></span><span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">Pré-Obra'+(gSt.obraFases.length>1?' F'+(obraFaseIdx+1):'')+' · '+du+' DU</span><span style="font-size:9px;opacity:.5;"> ✏</span></div>');
...
function getPreObraEfetivoByDay(faseIdx) {
  ...
  var discs = typeof _preObraMakeDiscs === 'function' ? _preObraMakeDiscs(poCfg.templateId) : tpl.disciplinas.filter(function(d){return d.ativo!==false;});
...
function gGetFontLevels() {
  var cfg = ESTADO && ESTADO.cfg;
  return {
    t1: (cfg && cfg.ganttT1fs) || 10,
    t2: (cfg && cfg.ganttT2fs) || 9,
    n1: (cfg && cfg.ganttN1fs) || 10,
    n2: (cfg && cfg.ganttN2fs) || 9
  };
}
window.gGetFontLevels = gGetFontLevels;

function gSetFontLevel(level, val) {
  val = Math.max(7, Math.min(16, parseInt(val) || 10));
  var key = 'gantt' + level + 'fs';
  if (!ESTADO.cfg) ESTADO.cfg = {};
  ESTADO.cfg[key] = val;
  document.documentElement.style.setProperty('--g' + level.toLowerCase() + '-fs', val);
  var styleId = 'gantt-typo-style';
  var el = document.getElementById(styleId) || document.createElement('style');
  el.id = styleId;
  var lv = gGetFontLevels();
  el.textContent = [
    /* T1: phaseHdr — Projetos, Obra */
    '#g-lbl-col > div[style*="text-transform:uppercase"][style*="height:20px"] { font-size: ' + lv.t1 + 'px !important; }',
    /* T2: sub-headers de fase — Arquitetura, Pré-Obra, Obra Total */
    '#g-lbl-col > div[style*="height:20px"][style*="font-size:9px"] { font-size: ' + lv.t2 + 'px !important; }',
    /* N1: disciplinas — linhas com SUB_H */
    '#g-lbl-col > div[style*="height:28px"], #g-lbl-col > div[style*="height:26px"] { font-size: ' + lv.n1 + 'px !important; }',
    /* N2: rótulos das barras */
    '#g-tl-inner .bar-label, #g-tl-inner [class*="bar"] span { font-size: ' + lv.n2 + 'px !important; }'
  ].join('\n');
  if (!el.parentNode) document.head.appendChild(el);
  onCfgChange(); salvarDados();
  var elDisp = document.getElementById('gantt-' + level.toLowerCase() + '-val');
  if (elDisp) elDisp.textContent = val;
}
window.gSetFontLevel = gSetFontLevel;

function gAplicarTipografiaGantt() {
  if (!ESTADO || !ESTADO.cfg) return;
  var lv = gGetFontLevels();
  ['T1','T2','N1','N2'].forEach(function(l) { gSetFontLevel(l, lv[l.toLowerCase()]); });
}
window.gAplicarTipografiaGantt = gAplicarTipografiaGantt;
