...
<th style="padding:5px 8px;text-align:left;font-family:var(--font);font-weight:700;letter-spacing:.05em;text-transform:uppercase;color:var(--txt-muted);border-bottom:1px solid var(--border);">Etapa</th>
<th style="padding:5px 8px;text-align:center;font-family:var(--font);font-weight:700;letter-spacing:.05em;text-transform:uppercase;color:var(--txt-muted);border-bottom:1px solid var(--border);width:90px;">Requer Aprov.</th>
<th style="padding:5px 8px;text-align:center;font-family:var(--font);font-weight:700;letter-spacing:.05em;text-transform:uppercase;color:var(--txt-muted);border-bottom:1px solid var(--border);width:60px;">Dias</th>
</tr></thead>
<tbody>
${ETAPAS_ARQ.map(e=>{const aprCfg=ESTADO.cfg?.aprovaCliente?.[e.id];const defAtiv=_ativListas.arq.find(a=>a.id===e.id)?.aprovaCliente??false;const defDias=_ativListas.arq.find(a=>a.id===e.id)?.diasAprova??1;const ativo=aprCfg?.ativo??defAtiv;const dias=aprCfg?.dias??defDias;return`<tr style="border-bottom:1px solid var(--border);"><td style="padding:5px 8px;font-family:var(--font);font-weight:700;color:var(--txt);">${e.label}</td><td style="padding:5px 8px;text-align:center;"><input type="checkbox" id="proj-f${t}-aprova-${e.id}" ${ativo?"checked":""} onchange="onAprovaChange(${t},'${e.id}')" style="accent-color:#D4A017;width:14px;height:14px;cursor:pointer;"></td><td style="padding:5px 8px;text-align:center;"><input type="number" id="proj-f${t}-aprova-dias-${e.id}" value="${dias}" min="1" max="10" onchange="onAprovaChange(${t},'${e.id}')" style="width:46px;padding:3px 5px;border:1px solid var(--border);border-radius:4px;background:var(--bg-surface);color:var(--txt);font-size:10px;text-align:center;" ${ativo?"":"disabled"}></td></tr>`;}).join("")}
</tbody></table></div>
...
function _pfRender() {
  ...
  subIds.forEach((subId,idx) => {
    ...
    // Dias de aprovação em dourado
    getDiasAprovacao(subId).forEach(iso => {
      if (etapasDoDia[iso]) etapasDoDia[iso] = {...etapasDoDia[iso], cor:'#D4A017', isAprova:true};
    });
  });
  ...
  const renderMes = (ano, mes) => {
    ...
    for (let cel=0; cel<total; cel++) {
      ...
      if (cel<primeiroDia||dia>ultimoDia) {
        ...
        if (etInfo) {
          const _aprv=etInfo.isAprova;
          cells+=`<td style="padding:1px;height:22px;" title="${_aprv?'Aprovação do cliente':(subNames[etInfo.subId]||'')}"><div style="background:${etInfo.cor};border-radius:3px;color:${_aprv?'#1A2535':'white'};font-size:9px;font-weight:600;text-align:center;padding:2px 0;${_aprv?'border:1px solid #B8860B;':''}opacity:${isDim?.25:1};${isDestaque?'outline:2px solid white;':''}${isHoje?'outline:2px solid var(--accent);':''}">${dia}</div></td>`;
        } else {
  ...
  const col4 = `
    <div style="display:flex;flex-direction:column;overflow:hidden;">
      ...
      <div style="padding:10px 12px;flex:1;overflow-y:auto;">
        ...
        <div style="display:flex;align-items:center;gap:5px;margin-bottom:8px;padding-bottom:8px;border-bottom:0.5px solid var(--border);">
          <div style="width:10px;height:10px;border-radius:2px;background:#D4A017;border:1px solid #B8860B;flex-shrink:0;"></div>
          <span style="font-size:10px;color:#D4A017;font-style:italic;">Aprovação do cliente</span>
        </div>
          ${legenda}
        </div>
      </div>
...
function getAprovaConfig(subId){const cfg=ESTADO.cfg?.aprovaCliente?.[subId];const def=_ativListas.arq.find(a=>a.id===subId);return{ativo:cfg?.ativo??(def?.aprovaCliente??false),dias:cfg?.dias??(def?.diasAprova??1)};}
function setAprovaConfig(subId,ativo,dias){if(!ESTADO.cfg.aprovaCliente)ESTADO.cfg.aprovaCliente={};ESTADO.cfg.aprovaCliente[subId]={ativo,dias};}
function getDiasAprovacao(subId){const fase=gSt.projFases[0];if(!fase)return[];const sub=fase.rows.arq.subs[subId];if(!sub)return[];const{ativo,dias}=getAprovaConfig(subId);if(!ativo||dias<=0)return[];const result=[];let cur=new Date(sub.end);while(result.length<dias){if(!CALENDARIO.isNaoUtil(cur))result.unshift(G.fmtISO(cur));cur=G.addD(cur,-1);if(G.ms(cur)<G.ms(sub.start))break;}return result;}
function gPopAprovaChange(subId){
  const chk=document.getElementById('pop-aprova-chk');
  const inp=document.getElementById('pop-aprova-dias');
  if(!chk||!inp)return;
  inp.disabled=!chk.checked;
  setAprovaConfig(subId,chk.checked,Math.max(1,parseInt(inp.value)||1));
  gRender();
}
function onAprovaChange(faseIdx,subId){const chk=document.getElementById(`proj-f${faseIdx}-aprova-${subId}`);const inp=document.getElementById(`proj-f${faseIdx}-aprova-dias-${subId}`);if(!chk||!inp)return;const ativo=chk.checked;const dias=Math.max(1,parseInt(inp.value)||1);inp.disabled=!ativo;setAprovaConfig(subId,ativo,dias);onCfgChange();gRender();}