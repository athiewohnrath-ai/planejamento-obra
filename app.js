// Planejamento de Obra A|W — v6.03.62
...
async function salvarDados(){try{lerUIparaEstado();const ts=Date.now(),payload={ts,estado:ESTADO};sessionStorage.setItem('aw_estado_atual',JSON.stringify(payload));if(_CRONO_ID){const el=document.getElementById('save-info');if(el)el.textContent='Salvando…';console.log('[Supabase] PATCH | cronoId='+_CRONO_ID);
        console.log('[Supabase] equipeARQ existe?', !!(ESTADO.cfg&&ESTADO.cfg.equipeARQ));
        console.log('[Supabase] planoAloc:', ESTADO.cfg&&ESTADO.cfg.equipeARQ&&ESTADO.cfg.equipeARQ.planoAloc?JSON.stringify(Object.keys(ESTADO.cfg.equipeARQ.planoAloc)):'VAZIO');
        console.log('[Supabase] gerentes:', ESTADO.cfg&&ESTADO.cfg.equipeARQ&&ESTADO.cfg.equipeARQ.gerentes);const r=await fetch(SB_URL+'/rest/v1/cronogramas?id=eq.'+_CRONO_ID,{method:'PATCH',headers:SB_HDR,body:JSON.stringify({codigo:ESTADO.meta.codigo||'',nome:ESTADO.meta.nome||'',gi:ESTADO.meta.gi||'',gp:ESTADO.meta.gp||'',estado_json:JSON.stringify(payload),atualizado_em:new Date().toISOString()})});console.log('[Supabase] resposta:',r.status,r.ok?'OK':'ERRO');const dt=new Date(ts).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});if(el)el.textContent=r.ok?'Salvo às '+dt:'Erro ao salvar';}}catch(e){console.error('salvarDados',e);}}
async function carregarDadosSB(){if(!_CRONO_ID)return false;const _cachedId=sessionStorage.getItem('aw_crono_id_cached');if(_cachedId&&_cachedId!==_CRONO_ID){sessionStorage.removeItem('aw_estado_atual');}sessionStorage.setItem('aw_crono_id_cached',_CRONO_ID);const cached=sessionStorage.getItem('aw_estado_atual');if(cached){try{const d=JSON.parse(cached);if(d.estado){const _hasPlano=d.estado.cfg&&d.estado.cfg.equipeARQ&&d.estado.cfg.equipeARQ.planoAloc&&Object.keys(d.estado.cfg.equipeARQ.planoAloc||{}).length>0;console.log('[App] carregando cache | ts:',d.ts,'| planoAloc:',_hasPlano?'presente':'AUSENTE');ESTADO=d.estado;window.__AW_ESTADO=ESTADO;estadoParaUI();
          // Verifica se localStorage tem equipeARQ mais recente (salvo pelo iframe)
          try{
            var _lid='aw_equipe_arq_'+_CRONO_ID;
            var _lraw=localStorage.getItem(_lid);
            if(_lraw){
              var _ldata=JSON.parse(_lraw);
              if(_ldata&&_ldata.equipeARQ&&_ldata.ts>d.ts){
                console.log('[App] localStorage tem equipeARQ mais recente — mesclando');
                if(!ESTADO.cfg)ESTADO.cfg={};
                ESTADO.cfg.equipeARQ=_ldata.equipeARQ;
