...
function _pfRender() {
  ...
  subIds.forEach((subId,idx) => {
    ...
    // Marcar dias de aprovação do cliente em dourado
    const aprDias = getDiasAprovacao(subId);
    aprDias.forEach(iso => {
      if (etapasDoDia[iso]) {
        etapasDoDia[iso] = {...etapasDoDia[iso], cor:'#D4A017', isAprova:true};
      }
    });
  });
  ...
  const renderMes = (ano, mes) => {
    ...
    for (let cel=0; cel<total; cel++) {
      ...
      if (cel<primeiroDia||dia>ultimoDia) {
        ...
        if (etInfo) {
          const isAprova = etInfo.isAprova;
          const txtColor = isAprova ? '#1A2535' : 'white';
          const border = isAprova ? 'border:1.5px solid #B8860B;' : '';
          const tooltip = isAprova ? 'Aprovação do cliente' : (subNames[etInfo.subId]||'');
          cells+=`<td style="padding:1px;height:22px;" title="${tooltip}"><div style="background:${etInfo.cor};border-radius:3px;color:${txtColor};font-size:9px;font-weight:600;text-align:center;padding:2px 0;${border}opacity:${isDim?.25:1};${isDestaque?'outline:2px solid white;':''}${isHoje?'outline:2px solid var(--accent);':''}">${dia}</div></td>`;
        } else {
    ...
    <div style="display:flex;flex-direction:column;overflow:hidden;">
      ...
      <div style="padding:10px 12px;flex:1;overflow-y:auto;">
        ...
        <div style="display:flex;align-items:center;gap:5px;margin-bottom:8px;padding-bottom:8px;border-bottom:0.5px solid var(--border);">
          <div style="width:10px;height:10px;border-radius:2px;background:#D4A017;flex-shrink:0;"></div>
          <span style="font-size:10px;color:var(--txt-muted);font-style:italic;">Aprovação do cliente</span>
        </div>
          ${legenda}
        </div>
      </div>
