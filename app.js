...
const _poCfg=ESTADO.cfg.obraFases&&ESTADO.cfg.obraFases[_poFaseIdx]&&ESTADO.cfg.obraFases[_poFaseIdx].preObra;
if(_poCfg&&_poCfg.ativo){
  const _poDu=_poCfg.du||5;
  let _poFim=G.addD(new Date(t.obra.start),-1);
  while(CALENDARIO.isNaoUtil(_poFim))_poFim=G.addD(_poFim,-1);
  let _poIni=new Date(_poFim),_poCount=1;
  while(_poCount<_poDu){_poIni=G.addD(_poIni,-1);if(!CALENDARIO.isNaoUtil(_poIni))_poCount++;}
  ...
  if(_tplPO){
    const _poDs=typeof _preObraMakeDiscs==='function'?_preObraMakeDiscs(_poCfg.templateId):_tplPO.disciplinas.filter(function(d){return d.ativo!==false;});
    _poDs.forEach(function(_pd,_pi){
      const _dc=typeof getDiscPal==='function'?getDiscPal(_pi):_poC;
      r.push('<div style="height:'+G.SUB_H+'px;background:#FEFCF5;display:flex;align-items:center;padding:0 8px;border-bottom:1px solid rgba(122,74,16,.12);border-right:2px solid #C0C8D4;font-size:9px;color:#6A3810;gap:4px;overflow:hidden;"><span style="width:4px;height:4px;border-radius:1px;background:'+_dc+';flex-shrink:0;"></span><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+(_pd.label||_pd.id)+'</span></div>');
      i.push('<div style="height:'+G.SUB_H+'px;background:#FEFCF5;position:relative;border-bottom:1px solid rgba(122,74,16,.12);overflow:hidden;">'+gGridLines(o,e,false)+'<div style="position:absolute;top:3px;left:'+_pxI+'px;height:'+(G.SUB_H-6)+'px;width:'+_pxW+'px;background:'+_dc+';border-radius:2px;opacity:.85;"></div></div>');
    });
  }
  spacer(2);
}
...
function gBuildAprovaHtml(t){if(!t||!t.subId||t.rowId!=='arq')return'';var ac=getAprovaConfig(t.subId);var sid=t.subId;return '<div style="margin-top:12px;border-top:1px solid #EEF0F4;padding-top:12px;"><div style="font-size:9px;font-weight:700;text-transform:uppercase;color:#D4A017;margin-bottom:6px;">★ Aprovação do Cliente</div><label style="display:flex;align-items:center;gap:7px;margin-bottom:7px;"><input type="checkbox" id="pop-aprova-chk" '+(ac.ativo?'checked ':'')+' onchange="gPopAprovaChange(''+sid+'')" style="accent-color:#D4A017;width:13px;height:13px;cursor:pointer;"><span style="font-size:12px;color:#3A4A5A;">Requer aprovação do cliente</span></label><label style="display:flex;align-items:center;gap:7px;"><span style="font-size:11px;font-weight:700;color:#8A95A8;width:36px;">Dias</span><input type="number" id="pop-aprova-dias" value="'+ac.dias+'" min="1" max="10" onchange="gPopAprovaChange(''+sid+'')" style="width:50px;padding:4px 6px;border:1px solid #C8CDD8;border-radius:4px;font-size:13px;text-align:center;" '+(ac.ativo?'':'disabled')+'></label></div>';}
