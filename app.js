...
function getAprovaConfig(subId){
  const cfg=ESTADO.cfg&&ESTADO.cfg.aprovaCliente&&ESTADO.cfg.aprovaCliente[subId];
  const def=_ativListas.arq.find(function(a){return a.id===subId;});
  return {
    ativo: cfg ? cfg.ativo : (def ? (def.aprovaCliente||false) : false),
    dias:  cfg ? cfg.dias  : (def ? (def.diasAprova||1)    : 1)
  };
}
function setAprovaConfig(subId,ativo,dias){
  if(!ESTADO.cfg.aprovaCliente)ESTADO.cfg.aprovaCliente={};
  ESTADO.cfg.aprovaCliente[subId]={ativo:ativo,dias:dias};
}
function getDiasAprovacao(subId){
  var fase=gSt.projFases[0];
  if(!fase)return[];
  var sub=fase.rows.arq.subs[subId];
  if(!sub)return[];
  var cfg=getAprovaConfig(subId);
  if(!cfg.ativo||cfg.dias<=0)return[];
  var result=[];
  var cur=new Date(sub.end);
  while(result.length<cfg.dias){
    if(!CALENDARIO.isNaoUtil(cur)){result.unshift(G.fmtISO(cur));}
    if(result.length<cfg.dias){cur=G.addD(cur,-1);if(G.ms(cur)<G.ms(sub.start))break;}
    else break;
  }
  return result;
}
function onAprovaChange(faseIdx,subId){
  var chk=document.getElementById('proj-f'+faseIdx+'-aprova-'+subId);
  var inp=document.getElementById('proj-f'+faseIdx+'-aprova-dias-'+subId);
  if(!chk||!inp)return;
...
function gPopAprovaChange(subId){
  var chk=document.getElementById('pop-aprova-chk');
  var inp=document.getElementById('pop-aprova-dias');
  if(!chk||!inp)return;
  inp.disabled=!chk.checked;
  setAprovaConfig(subId,chk.checked,Math.max(1,parseInt(inp.value)||1));
  gRender();
