...
(function(obraFase, obraFaseIdx){
  ...
  // Disciplinas
  if(tpl){
    var ds=(_custom&&_custom.disciplinas&&_custom.disciplinas.length)?_custom.disciplinas.filter(function(d){return d.ativo!==false;}):(typeof _preObraMakeDiscs==='function'?_preObraMakeDiscs(pc.templateId):(tpl&&tpl.disciplinas?tpl.disciplinas.filter(function(d){return d.ativo!==false;}):[]));  
...
function gClosePop(){
  ...
  gClosePopForce();
    ...
    n = '<div style="display:flex;gap:6px;align-items:center;margin-bottom:14px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#1A2535;">'
      ...
      + '<span style="flex:1;border:1px solid #E8E0D4;border-radius:4px;padding:5px 8px;font-size:12px;background:#F8F4EE;color:#6A3810;">' + G.fmtBR(_poIni) + '</span></label>'
      + '<label style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">'
      + '<span style="width:38px;font-size:9px;font-weight:700;text-transform:uppercase;color:#8A95A8;">Fim</span>'
      + '<span style="flex:1;border:1px solid #E8E0D4;border-radius:4px;padding:5px 8px;font-size:12px;background:#F8F4EE;color:#6A3810;">' + G.fmtBR(_poFim) + '</span></label>'
      ...
      + '<button onclick="_poPopAdjust(' + _poFi + ',-5)" style="width:38px;height:100%;border:none;border-right:1px solid #C8CDD8;background:#F4F6F8;cursor:pointer;font-size:14px;color:#4A5268;">−</button>'
      + '<button onclick="_poPopAdjust(' + _poFi + ',-1)" style="width:34px;height:100%;border:none;border-right:1px solid #C8CDD8;background:#F8F9FB;cursor:pointer;font-size:12px;color:#4A5268;">−1</button>'
      + '<div style="flex:1;text-align:center;"><span style="font-size:18px;font-weight:800;color:' + _poColor + ';">' + _poDu + '</span>'
      + '<span style="font-size:9px;font-weight:700;color:#A0A8B8;margin-left:2px;">dias úteis</span></div>'
      + '<button onclick="_poPopAdjust(' + _poFi + ',1)" style="width:34px;height:100%;border:none;border-left:1px solid #C8CDD8;background:#F8F9FB;cursor:pointer;font-size:12px;color:#4A5268;">+1</button>'
      + '<button onclick="_poPopAdjust(' + _poFi + ',5)" style="width:38px;height:100%;border:none;border-left:1px solid #C8CDD8;background:#F4F6F8;cursor:pointer;font-size:14px;color:#4A5268;">+</button>'
      + '</div>'
...
function abrirObraUnificada(phId) {
  ...
  ouRender();
  document.getElementById('modal-ou-overlay').style.display = 'flex'; ouSwitchTab('arq');
}

function ouSwitchTab(tab) {
  var arqTab = document.getElementById('ou-tab-arq');
  var tecTab = document.getElementById('ou-tab-tec');
  var arqContent = document.getElementById('ou-content');
  var tecContent = document.getElementById('ou-tec-content');
  var btn = document.getElementById('ou-btn-fechar');
  if (!arqTab) return;
  if (tab === 'arq') {
    arqTab.style.borderBottomColor = 'var(--accent,#00AEDF)'; arqTab.style.color = 'var(--txt)';
    tecTab.style.borderBottomColor = 'transparent'; tecTab.style.color = 'var(--txt-muted)';
    if (arqContent) arqContent.style.display = 'block';
    if (tecContent) tecContent.style.display = 'none';
    if (btn) btn.textContent = 'Próximo →';
  } else {
    tecTab.style.borderBottomColor = 'var(--accent,#00AEDF)'; tecTab.style.color = 'var(--txt)';
    arqTab.style.borderBottomColor = 'transparent'; arqTab.style.color = 'var(--txt-muted)';
    if (arqContent) arqContent.style.display = 'none';
    if (tecContent) tecContent.style.display = 'block';
    if (btn) btn.textContent = 'Salvar e Fechar';
    tecFornRenderModal();
  }
}
window.ouSwitchTab = ouSwitchTab;
...
function gGetTypo(level) {
  ...
  return { fs: cfg[level].fs || def.fs, fw: cfg[level].fw || def.fw, ff: cfg[level].ff || def.ff };
}
...
function gSetTypo(level, prop, val) {
  ...
  gAplicarTypo(); onCfgChange(); salvarDados();
}
...
function gAplicarTypo() {
  ...
  ['T1','T2','N1','N2'].forEach(function(lv) {
    var t = gGetTypo(lv); var cls = lv.toLowerCase();
    rules.push('.g'+cls+', .g'+cls+' span {font-size:'+t.fs+'px !important;font-weight:'+t.fw+' !important;font-family:'+t.ff+' !important;'+(cls==='gn1'?'text-transform:none !important;letter-spacing:normal !important;':'')+'}');
  });
  el.textContent = rules.join('\n');
  if (!el.parentNode) document.head.appendChild(el);
  ['T1','T2','N1','N2'].forEach(function(lv) {
    var t = gGetTypo(lv); var cls = lv.toLowerCase();
    var elFs = document.getElementById('gt-'+cls+'-fs'); if (elFs) elFs.textContent = t.fs;
    var elFw = document.getElementById('gt-'+cls+'-fw'); if (elFw) elFw.value = t.fw;
    var elFf = document.getElementById('gt-'+cls+'-ff'); if (elFf) elFf.value = t.ff;
  });
}
window.gAplicarTypo = gAplicarTypo;

// ── Resize da coluna lateral ──
function gInitColResize() {
  ...
  handle.addEventListener('mousedown', function(e) {
    ...
    function onMove(e) {
      var dx = e.clientX - startX; var newW = Math.max(120, Math.min(320, startW + dx));
      if (!ESTADO.cfg) ESTADO.cfg = {}; ESTADO.cfg.lblW = newW; gRender();
    }
    function onUp() {
...
function gAplicarFontes() {
  if (!ESTADO || !ESTADO.cfg) return;
  if (ESTADO.cfg.ganttTypo) gAplicarTypo();
}
  ...
  var discsHtml = discs.map(function(disc, di) {
    var tasksHtml = (disc.tasks||[]).map(function(task, ti) {
      return '<div style="display:flex;align-items:center;gap:6px;margin-bottom:5px;padding:5px 8px;background:var(--bg-surface);border-radius:4px;border:0.5px solid var(--border);">'
        ...
        + '<button onclick="_poMoverTask('+faseIdx+','+di+','+ti+',-1)" style="width:20px;height:22px;background:var(--bg-surface2);border:0.5px solid var(--border);border-radius:3px;cursor:pointer;font-size:11px;" title="Subir">↑</button>'+'<button onclick="_poMoverTask('+faseIdx+','+di+','+ti+',1)" style="width:20px;height:22px;background:var(--bg-surface2);border:0.5px solid var(--border);border-radius:3px;cursor:pointer;font-size:11px;" title="Descer">↓</button>'+'<button onclick="_poRemoverTarefa('+faseIdx+','+di+','+ti+')" '
    ...
    return '<div data-po-disc="'+di+'" draggable="true" style="border:1px solid var(--border);border-radius:8px;margin-bottom:10px;overflow:hidden;">'
      ...
      + '<button onclick="_poMoverDisc('+faseIdx+','+di+',-1)" style="width:22px;height:26px;background:var(--bg-surface2);border:0.5px solid var(--border);border-radius:4px;cursor:pointer;font-size:12px;" title="Subir">↑</button>'+'<button onclick="_poMoverDisc('+faseIdx+','+di+',1)" style="width:22px;height:26px;background:var(--bg-surface2);border:0.5px solid var(--border);border-radius:4px;cursor:pointer;font-size:12px;" title="Descer">↓</button>'+'<button onclick="_poRemoverDisc('+faseIdx+','+di+')" '
...
function _poMoverDisc(faseIdx, di, dir) {
  ...
  var j = di + dir;
  if (j < 0 || j >= discs.length) return;
  var tmp = discs[di]; discs[di] = discs[j]; discs[j] = tmp;
  onCfgChange(); _poRenderModal(faseIdx);
}
window._poMoverDisc = _poMoverDisc;

function _poMoverTask(faseIdx, di, ti, dir) {
  var tasks = ESTADO.preObraCustom[faseIdx].disciplinas[di].tasks;
  var j = ti + dir;
  if (j < 0 || j >= tasks.length) return;
  var tmp = tasks[ti]; tasks[ti] = tasks[j]; tasks[j] = tmp;
  onCfgChange(); _poRenderModal(faseIdx);
}
window._poMoverTask = _poMoverTask;
