...
const PF_ARQUITETOS_DEFAULT = [
  'Ana Lima','Bruno Costa','Carol Mendes','Diego Rocha',
  'Elena Souza','Felipe Nunes','Gabriela Pires','Henrique Alves'
];
...
// Estado do plano fino
let _pfDados = null;
let _pfEtapaSel = null; // subId da etapa selecionada na col 2
let _pfArqDrag = null;  // índice do arquiteto sendo arrastado

function _pfInicializar() {
  // Carregar do ESTADO ou criar novo
  if (ESTADO.planoFino) {
    _pfDados = ESTADO.planoFino;
  } else {
    _pfDados = {
      arquitetos: PF_ARQUITETOS_DEFAULT.map((nome, i) => ({
        id: 'arq_' + i, nome, cor: PF_CORES[i % PF_CORES.length]
      })),
      etapas: {} // {subId: {arquitetos: [id,...], tarefas: {nome: bool}}}
    };
  }
  _pfEtapaSel = null;
}
...
function _pfRender() {
  ...
  // Col 1: arquitetos
  let col1 = _pfDados.arquitetos.map((arq, i) => {
    const iniciais = arq.nome.split(' ').map(p => p[0]).join('').slice(0,2).toUpperCase();
    return `<div class="pf-arq-card" draggable="true"
      ondragstart="_pfDragStart(${i},event)"
      ondragend="_pfDragEnd(event)"
      style="display:flex;align-items:center;gap:8px;padding:7px 10px;border-radius:7px;border:0.5px solid var(--border);cursor:grab;background:var(--bg-panel);margin-bottom:5px;">
      <svg width="10" height="14" viewBox="0 0 10 14" fill="none" style="opacity:.4;flex-shrink:0;">
        ...
        <circle cx="3" cy="12" r="1.2" fill="currentColor"/><circle cx="7" cy="12" r="1.2" fill="currentColor"/>
      </svg>
      <div style="width:28px;height:28px;border-radius:50%;background:${arq.cor}22;border:1.5px solid ${arq.cor};display:flex;align-items:center;justify-content:center;font-family:var(--font);font-size:10px;font-weight:700;color:${arq.cor};flex-shrink:0;">${iniciais}</div>
      <span style="font-size:12px;color:var(--txt);flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${arq.nome.split(' ')[0]}</span>
    </div>`;
  }).join('') + `<button onclick="_pfAddArquiteto()" style="margin-top:6px;font-size:11px;color:var(--txt-muted);background:none;border:none;cursor:pointer;padding:4px 2px;text-align:left;">+ novo arquiteto</button>`;
  ...
  subIds.forEach(subId => {
    ...
    const chips = arqsNaEtapa.map(arq => {
      const ini = arq.nome.split(' ').map(p=>p[0]).join('').slice(0,2).toUpperCase();
      return `<div style="display:flex;align-items:center;gap:4px;background:${arq.cor}18;border:1px solid ${arq.cor}44;padding:2px 8px;border-radius:20px;">
        <div style="width:16px;height:16px;border-radius:50%;background:${arq.cor}33;display:flex;align-items:center;justify-content:center;font-size:8px;font-weight:700;color:${arq.cor};">${ini}</div>
        <span style="font-size:11px;color:${arq.cor};">${arq.nome.split(' ')[0]}</span>
        <span onclick="_pfRemArqEtapa('${subId}','${arq.id}')" style="font-size:10px;cursor:pointer;color:${arq.cor};margin-left:2px;opacity:.7;">✕</span>
      </div>`;
    }).join('');

    const dropZone = arqsNaEtapa.length === 0
      ? `<div class="pf-drop-zone" data-subid="${subId}"
          ...
          style="border:1.5px dashed var(--border-md);border-radius:6px;padding:7px 12px;display:flex;align-items:center;justify-content:center;gap:6px;color:var(--txt-dim);font-size:11px;margin-top:4px;">
          ...
          solte arquitetos aqui
        </div>`
      : `<div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:6px;" class="pf-drop-zone" data-subid="${subId}"
          ondragover="_pfDragOver(event)" ondrop="_pfDrop('${subId}',event)" ondragleave="_pfDragLeave(event)">
          ${chips}
          <div style="border:1px dashed var(--border-md);padding:2px 8px;border-radius:20px;font-size:11px;color:var(--txt-dim);cursor:pointer;">+ soltar aqui</div>
        </div>`;

    col2 += `<div onclick="_pfSelEtapa('${subId}')"
      style="padding:10px 14px;border-bottom:0.5px solid var(--border);cursor:pointer;${isSel ? 'background:rgba(0,222,219,.06);border-left:2px solid var(--accent);' : ''}transition:background .12s;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:${arqsNaEtapa.length > 0 ? 6 : 0}px;">
        <div>
          <span style="font-family:var(--font);font-size:13px;font-weight:700;color:${isSel ? 'var(--accent)' : 'var(--txt)'};">${nome}</span>
          <span style="font-size:11px;color:var(--txt-muted);margin-left:8px;">${fmtD(sub.start)} – ${fmtD(sub.end)} · ${du} DU</span>
        </div>
        <svg width="12" height="12" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="1.5" style="opacity:.4;"><path d="M4 2l4 4-4 4"/></svg>
      </div>
      ${dropZone}
    </div>`;
  });

  // Col 3: tarefas da etapa selecionada
  let col3 = '';
  if (_pfEtapaSel) {
    const nome = subNames[_pfEtapaSel] || _pfEtapaSel;
    ...
    const renderTarefa = (t) => {
      ...
      return `<div style="display:flex;align-items:center;gap:8px;padding:7px 10px;border-radius:6px;border:0.5px solid ${ativa ? 'var(--accent)' : 'var(--border)'};background:${ativa ? 'rgba(0,222,219,.06)' : 'transparent'};margin-bottom:4px;cursor:pointer;"
        onclick="_pfToggleTarefa('${_pfEtapaSel}','${t.replace(/'/g, "\\'")}')">
        <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="${ativa ? 'var(--accent)' : 'var(--txt-dim)'}" stroke-width="1.5">
          ${ativa ? '<rect x="1" y="1" width="12" height="12" rx="2"/><path d="M3.5 7l2.5 2.5 4.5-4.5"/>' : '<rect x="1" y="1" width="12" height="12" rx="2"/>'}
        </svg>
        <span style="font-size:12px;color:${ativa ? 'var(--txt)' : 'var(--txt-muted)'};">${t}</span>
      </div>`;
    };

    col3 = `<div style="padding:8px 12px;font-family:var(--font);font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--accent);border-bottom:0.5px solid var(--border);background:rgba(0,222,219,.04);">${nome}</div>
    <div style="padding:10px 12px;flex:1;overflow-y:auto;">
      ${tarefasPadrao.map(renderTarefa).join('')}
      ${tarefasCustom.map(renderTarefa).join('')}
      <div style="margin-top:10px;border-top:0.5px solid var(--border);padding-top:10px;">
        <div style="display:flex;gap:6px;">
          <input id="pf-nova-tarefa" type="text" placeholder="Nova tarefa..." onkeydown="if(event.key==='Enter')_pfAddTarefa()"
            style="flex:1;height:32px;padding:0 10px;border:0.5px solid var(--border);border-radius:6px;background:var(--bg-surface);color:var(--txt);font-size:12px;outline:none;">
          <button onclick="_pfAddTarefa()" style="height:32px;padding:0 12px;background:var(--accent);border:none;border-radius:6px;font-family:var(--font);font-size:10px;font-weight:700;color:#0D1117;cursor:pointer;">Ok</button>
        </div>
      </div>
    </div>`;
  } else {
    col3 = `<div style="flex:1;display:flex;align-items:center;justify-content:center;padding:24px;text-align:center;">
      <div>
        <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="var(--txt-dim)" stroke-width="1.2" style="margin-bottom:10px;"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/><path d="M9 12h6M9 16h4"/></svg>
        <p style="font-size:12px;color:var(--txt-dim);">Selecione uma etapa<br>para ver as tarefas</p>
      </div>
    </div>`;
  }

  body.innerHTML = `
    <div style="display:grid;grid-template-columns:190px 1fr 220px;height:100%;min-height:0;">
      <!-- Col 1 -->
      <div style="border-right:0.5px solid var(--border);display:flex;flex-direction:column;overflow:hidden;">
        <div style="padding:8px 14px;font-family:var(--font);font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--txt-muted);background:var(--bg-surface);border-bottom:0.5px solid var(--border);">Arquitetos</div>
        <div style="padding:10px 12px;flex:1;overflow-y:auto;">
          <p style="font-size:11px;color:var(--txt-dim);margin-bottom:10px;">Arraste para a etapa</p>
          ${col1}
        </div>
      </div>
      <!-- Col 2 -->
      <div style="display:flex;flex-direction:column;overflow:hidden;border-right:0.5px solid var(--border);">
        <div style="padding:8px 14px;font-family:var(--font);font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--txt-muted);background:var(--bg-surface);border-bottom:0.5px solid var(--border);">Etapas</div>
        <div style="flex:1;overflow-y:auto;">${col2}</div>
      </div>
      <!-- Col 3 -->
      <div style="display:flex;flex-direction:column;overflow:hidden;">
        <div style="padding:8px 14px;font-family:var(--font);font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--txt-muted);background:var(--bg-surface);border-bottom:0.5px solid var(--border);">Tarefas</div>
        <div style="flex:1;overflow-y:auto;display:flex;flex-direction:column;">${col3}</div>
      </div>
    </div>`;
}
...
function _pfDragEnd(e) {
  e.currentTarget.style.opacity = '1';
  _pfArqDrag = null;
}
function _pfDragOver(e) {
  e.preventDefault();
  e.currentTarget.style.background = 'rgba(0,222,219,.08)';
  e.currentTarget.style.borderColor = 'var(--accent)';
}
function _pfDragLeave(e) {
...
function _pfToggleTarefa(subId, tarefa) {
  if (!_pfDados.etapas[subId]) return;
  const atual = _pfDados.etapas[subId].tarefas[tarefa];
  _pfDados.etapas[subId].tarefas[tarefa] = !atual;
  _pfSalvar();
  _pfRender();
}
...
function _pfAddArquiteto() {
  const nome = prompt('Nome do arquiteto:');
  if (!nome?.trim()) return;
  const i = _pfDados.arquitetos.length;
  _pfDados.arquitetos.push({ id: 'arq_' + Date.now(), nome: nome.trim(), cor: PF_CORES[i % PF_CORES.length] });
  _pfSalvar();
  _pfRender();
}
...
function gClosePop(){
  ...
  gClosePopForce();
    ...
    // Mostrar marca d'água e botão Plano Fino se congelado
    try{
      const _xid=sessionStorage.getItem('aw_crono_id');
      if(_xid){
        const _xi=JSON.parse(localStorage.getItem('aw_crono_index')||'[]');
        const _xe=_xi.find(x=>x.id===_xid);
        if(_xe&&_xe.status==='frozen'){
